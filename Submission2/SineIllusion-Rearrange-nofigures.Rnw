...
require(knitr)
opts_chunk$set(fig.path='figure/fig-', cache.path='cache/', fig.align='center', fig.width=5, fig.height=5, fig.show='hold', par=TRUE, cache=TRUE, concordance=TRUE, autodep=TRUE, eval=FALSE, include=FALSE)
library(reshape2)
...
% ggplot( data=subset(monthly, year(month)>=1995) ) + geom_point(aes(month, sd), color="grey40") + ylab("Standard deviation of daily gas prices")  + xlab("Time")+ theme_bw() + #ggtitle("Standard Deviation of Gas Prices")
%   theme(plot.margin=unit(c(0,.5,0,0), "cm")) + geom_smooth(aes(month, sd), method="loess", color="black") + ylim(c(0,0.17))
% @
% \end{SCfigure}
