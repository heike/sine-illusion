...
@
\begin{figure}[h!tbp]
<<gasprices-pictures,echo=F, include=T, fig.width=8, fig.height=2.75, out.width=".98\\textwidth", dependson='gasprices'>>=
...
as well as several dramatic price changes. These developments mask the steady increase in variance shown in Figure \ref{fig:gasvariance}.
