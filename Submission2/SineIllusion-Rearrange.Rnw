...
\begin{figure}[htbp]\centering
<<meanDists,echo=FALSE,out.width='.65\\linewidth', dependson='results-analysis', fig.width=7, fig.height=4, message=FALSE, warning=FALSE>>=
dt.int <- ddply(user.avg.all, .(type), summarize, overall.avg=unique(overall.avg), sd=sd(avg))
model <- lm(avg~type-1, data=user.avg.all)
temp <- as.data.frame(cbind(coef(model), confint(model)))
temp$CI <- sprintf("(%.04f, %.04f)", temp[,2], temp[,3])
...
We can therefore interpret $\beta$ as the length of the interval of plausible weights. Additionally, we can interpret the quantity $(\alpha_* + \beta/2)$ as equivalent to the estimate of the optimal weight using the psychophysics methodology.
...
\caption{Raw monthly maximum temperatures (in Celsius) between 2008 and 2012 at 380 stations across the US (as indicated on the map) with similar elevation and latitude. The line shows fitted temperatures from a baseline model. The plot suggests that the model fails to capture the more extreme values in summer and winter. \label{fig:raw-temp}}
\end{figure}
