...
, though usually not as an optical illusion. Rather, the problem is typically identified as the difficulty of visually subtracting two curves \citep[p. XXX]{robbins:2005}, and the resulting erroneous conclusions when this process goes awry. \newtext{\citet[p. XXX]{cleveland:1984} also noted the illusion in the context of assessing variability of points along a trend line in scatterplots.}  Playfair's chart of the balance of trade between England and the East Indies \citep{playfair, playfair2} (shown in Appendix \ref{appendixgraphics}) represents the possibly oldest example of this common phenomenon. 
...
Recreating the three-dimensional context of the sine illusion might resolve the distortion. Generally, an increase of the dimensionality of a graph is  not recommended \citep{tufte, cleveland:1984}, but \citet{spence:1990} suggests that %in certain simple cases additional dimensions are not misleading.
\newtext{adding a third dimension to simple statistical graphics does not interfer with an accurate reading.}
...
#   geom_rug(data=dots, aes(x=x))+
  ...
  theme(plot.margin=unit(c(0,0,0,0), "cm"), axis.text.x=element_blank(), axis.ticks.x=element_blank())
...
\caption{Corrected gas price data using $X$-transformations with $w=1$ and  $w=0.36$. \newdo{the labels on the x axis aren't right - we need two separate x axis with labels - the vertical grid lines are also wrong. }}
