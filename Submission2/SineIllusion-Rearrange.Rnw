...
%  \includegraphics[width=.68\textwidth]{figure/fig-example1}
  \includegraphics[height=2in]{figure/fig-example1}
...
\begin{subfigure}[b]{.44\linewidth}
  \centering
  \includegraphics[height=2in]{figure/fig-example2}
...
, though usually not as an optical illusion. Rather, the problem is typically identified as the difficulty of visually subtracting two curves \citep[p. XXX]{robbins:2005}, and the resulting erroneous conclusions when this process goes awry. \citet[p. XXX]{cleveland:1984} also noted the illusion in the context of assessing variability of points along a trend line in scatterplots.  Playfair's chart of the balance of trade between England and the East Indies \citep{playfair, playfair2} (shown in Appendix \ref{appendixgraphics}) represents the possibly oldest example of this common phenomenon. 
...
\includegraphics[width=3in, keepaspectratio=TRUE, trim=0in 1.75in 0in 1.75in]{figure/fig-ribbon-illusion1}
\caption{Perspective plot of sine illusion\label{ribbon1}}
\end{subfigure}
\begin{subfigure}[t]{.54\linewidth}\centering
\includegraphics[width=3in, keepaspectratio=TRUE, trim=0in 1.75in 0in 1.75in]{figure/fig-ribbon-illusion2}
...
\begin{figure}[h!tbp]
\vspace{-.2in}
\includegraphics[width=.85\linewidth, keepaspectratio=TRUE]{fig-transform-illustration}
\centering
\vspace{-.3in}
...
\includegraphics[keepaspectratio=TRUE,width=0.9\textwidth]{figure/fig-generalcorrectioncartoon1}
...
It is of additional interest to determine whether and how much these optimal weights are subject-specific or population-based, whether they depend on the initial weight, and how much within-subject variability we find compared to between-subject variability. The results of the analysis suggest that the correction factors in $X$ and $Y$ are both preferable to uncorrected data, but that a full correction is not necessary to break the illusion.
...
The study aims to determine the range of ``optimal'' transformation weights for each transformation type. Psychophysics methodology typically approaches threshold estimation by using the method of adjustment \citep{goldstein}, where  stimuli are provided showing states both above and below the hypothesized optimal value and  participants adjust the stimuli until the stated goal is met (in this case, until the lines appear to have equal length). It is expected that there will be a difference in user-reported values from below and from above, and these values are typically averaged to produce a single threshold value (the results from this model are provided in appendix \ref{psychophysics}). Instead of averaging these values, we use a mixed model to compare user responses for different starting points %in a more continuous fashion, incorporating some of the advantages of the method of constant stimuli to more robustly estimate 
to be able to estimate the {\it range of  transformation weights}. % For a review of general psychophysics methodology, the method of adjustment, and the method of constant stimuli, see \citet{goldstein}.
...
\includegraphics[keepaspectratio=TRUE,width=0.7\linewidth]{figure/fig-study-weight-dist}
...
\begin{figure}[hbtp]
\centering
\includegraphics[keepaspectratio=TRUE, width=.85\linewidth]{figure/fig-results-analysis1}
...
ggplot(data=gas.cor.x) + 
  ...
  geom_text(data=axis.labs, aes(x=value, y=0, label=text), hjust=.25) + 
...
\begin{SCfigure}[2][htbp]
\centering
\includegraphics[keepaspectratio=TRUE,width=.7\linewidth]{figure/gas-x-corrected}
...
\begin{SCfigure}[2][htbp]
\centering
\includegraphics[keepaspectratio=TRUE,width=.7\linewidth]{figure/gas-y-corrected}
...
\includegraphics[keepaspectratio=TRUE,width=.8\linewidth]{figure/fig-overcorrectlimits}
