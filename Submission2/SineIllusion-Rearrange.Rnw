...
@
\caption{US Gas prices, 1995-2014 with a trendline fit using smothing splines, which provide first and second derivatives necessary for the corrections proposed in this paper. The nonlinear trend masks the increasing variance in daily gas prices over time.}\label{fig:gasprices}
\end{figure}
We will demonstrate the occurrence of the sine illusion as well as the suggested transformations using data gathered from the Energy Information Administration's historical database of gas prices \citep{EIA}. This data, collected from 1995-2014, includes prices for all three grades of gasoline as well as two chemical formulations which are sold in different geographic areas (for more information, see \citep{EIA-reformulated}. Figure \ref{fig:gasprices} shows the gas prices over the period of interest; there is a clearly nonlinear trend to the data, with several dramatic price changes. This trend masks the increasing variance shown in Figure \ref{fig:gasvariance}.

\begin{SCfigure}\centering
\caption{Std. deviation of daily gas prices, 1995-2014. There is a clear increasing trend in the variance that is masked by the increasing price per gallon in Figure \ref{fig:gasprices}.\label{fig:gasvariance}}
<<gaspricevariance,echo=F,include=T, fig.width=6, fig.height=2, out.width=".6\\textwidth", dependson='gasprices'>>=
ggplot( data=subset(monthly, year(month)>=1995) ) + geom_line(aes(month, sd)) + ylab(expression(sigma))  + xlab("Time")+ theme_bw() + ggtitle("Standard Deviation of Gas Prices")
@
\end{SCfigure}
