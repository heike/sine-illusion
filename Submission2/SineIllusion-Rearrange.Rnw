...
<<gaspricevariance,echo=F,include=T, fig.width=6, fig.height=3, out.width=".7\\textwidth", dependson='gasprices'>>=
ggplot( data=subset(monthly, year(month)>=1995) ) + geom_line(aes(month, sd)) + ylab("Standard deviation of daily gas prices")  + xlab("Time")+ theme_bw() + #ggtitle("Standard Deviation of Gas Prices")
