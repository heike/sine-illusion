...
What follows is a compilation of several approaches to correct for or mitigate the effect of the illusion. Our primary intention here is to demonstrate the pervasiveness of the illusion is and the extreme measures necessary to remove its effect. 
...
\item transformation of $x$: adjusting the slope to be constant by reparameterizing the $x$ axis, and
\item transformation of $y$: adjusting $y$ values to make conditional variability  appear correctly by adjusting according to orthogonal width. 
\end{enumerate}
