...
@
\begin{figure}
<<gasprices-pictures,echo=F, include=T, fig.width=8, fig.height=3, out.width=".98\\textwidth", dependson='gasprices'>>=
p1 <- ggplot(data=gasprices)  +
...
@
\caption{US Gas prices, 1995-2014. The left graph shows gas prices over the time period with a trendline fit using smoothing splines, which provide first and second derivatives necessary for the corrections proposed in this paper. The nonlinear trend masks the increasing variance in daily gas prices over time shown in the right graph.}\label{fig:gasprices}
\end{figure}
\begin{figure}\centering
<<gaspricesMonthlyRange,echo=F, fig.width=8, fig.height=3, out.width='\\textwidth'>>=
ggplot(data=monthly)  +
  geom_linerange(aes(x=month, ymin=min, ymax=max, y=median)) +
  ylab("Price per gallon (USD)") + theme_bw()+ xlab("Time") + 
  ggtitle("Monthly Range of Gas Prices, 1995-2014")
@
\caption{Monthly Range of Gas Prices, 1995 - 2014. There is some variation in the length of the lines, most notably when prices change dramatically, but the magnitude of the long-term change in variation is obscured by the trend. In fact, the standard deviation of seasonal prices in 2013 and 2014 ($0.142-$0.152) is approximately twice the standard deviation of seasonal prices in 1995 ($0.077-$0.079).}\label{fig:pricerange}
\end{figure}
Figure \ref{fig:pricerange} shows the monthly range of gas prices across the US. 

<<gaspricesCorrection, echo=F, cache=T>>=
