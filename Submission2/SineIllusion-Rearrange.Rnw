...
\newtext{These types of plots are often used to give an overview of the relationship between two variables. The trendline summarizes this relationship, while the points show raw measurement to allow an assessment of the overall size of the data, the amount of (marginal) variability presented, as well as the (conditional) variability along the trendline. It is the latter task that we cannot answer satisfactorily. While}
%Does the variability of ozone concentrations stay the same across the range of temperatures? 
...
\includegraphics[keepaspectratio=true, width=.4\linewidth]{figure/fig-original}
\end{SCfigure}
...
\newtext{Additionally, the illusion itself is insidious - we trust our vision implicitly, to the point that when we understand something, we say ``I see''. This trust in our visual perception is seldom called into question, for our perception is optimized for interaction with a three-dimensional world. Artificially two-dimensional situations (such as graphs and pictures) may accurately represent the data and still produce a misleading perceptual experience.}
...
<<transform-illustration,echo=FALSE, message=FALSE, warning=FALSE, include=FALSE, fig.width=6.25, fig.height=5>>=
f <- function(x) 2*sin(x)
fprime <- function(x) 2*cos(x)
f2prime <- function(x) -2*sin(x)
library(plyr)
...
qplot(x=x, y=y, geom="line", data=dframe, colour=I("grey50")) + theme_bw() + 
  ...
  theme(legend.key.width = unit(3, "line"), plot.margin = unit(c(0,0,0,0), "cm"), legend.position=c(.2, .2)) 
@
...
\includegraphics[width=.5\linewidth, keepaspectratio=TRUE]{fig-transform-illustration}
\end{SCfigure}
...
In this  case it may be  possible to correct the graphical display for the illusion to minimize its misleading effect. A geometrical correction that  --at least temporarily-- counteracts the illusion is be a valuable tool in visual analysis, as this illusion very persistently affects our judgment of very common tasks  such as e.g.~the assessment of conditional variability of data along a trend line.
...
What follows is a compilation of several approaches to correct for or mitigate the effect of the illusion. Our primary intention here id to demonstrate the persaviness of the illusion is and the extreme measures necessary to remove its effect. 
...
In the following two sections we are making use of Taylor approximations of first and second order to find approximate solutions to  end points as sketched out in figure \ref{fig:linear.quadratic}.
...
%Depending on whether the illusion is present at the starting value, 
Users are asked to adjust the graph until the illusion  (a) is no longer apparent (adjustment of weights from the bottom) or (b) becomes visible (adjustments of weights from the top).
This adjustment is done by clicking on a plus/minus button to either increase or decrease the amount of correction used. Underlying this adjustment is the value of the weight $w$ as defined in eqns.~(\ref{eqn.xtrans.weighted}) and~(\ref{eqn.ytrans.weighted}). The numerical value of $w$ was hidden from the user to prevent anchoring to a specific numerical value.
A low  initial weight ($w_0$ close to 0) indicates that the amount of correction is low and the response from a trial like this will give us an idea of the minimal amount of weight necessary to break the illusion, while a high initial weight ($w_0$ close to 1) indicates that the data is fully corrected. We asked participants to reduce the amount of adjustment until the lines appear to be the same length assumes that the correction is overcorrecting in practice, and a response from this type of trial gives us an upper boundary for the amount of weighting preferred. Generally, responses from the two different types of trials do not result in the same threshold weight, but rather lead to a range of acceptable weights.
...
%Each transformation type has a lower bound (corresponding to starting at $w=0$) and an upper bound (corresponding to starting at $w=1$); these values are derived from the linear model parameters, where the lower bound is $\alpha_{T(i,j)}$ and the upper bound is $\alpha_{T(i,j)}+\beta X_{ij}$. 
...
ggplot(data=gasprices)  +
  geom_jitter(aes(decimal_date(date), price), data=subset(gasprices, year>=1995), size=1.5, shape=I(1), colour=I("grey40"), alpha=I(.75))+ 
  geom_line(data=gas.pred, aes(x=x, y=y), inherit.aes=FALSE) + 
  ylab("Price per gallon (USD)") + theme_bw()+ xlab("Time")  + geom_point(aes(x=1995, y=0), size=0.1)
#  ggtitle("Price of Gasoline in the US, 1995-2014")
@
\caption{US Gas prices from 1995 to 2014. The dark trendline fit uses smoothing splines, and provides first and second derivatives necessary for the proposed corrections. The nonlinear trend masks the increasing variance in daily gas prices over time.}\label{fig:gasprices}
\end{figure}
We will demonstrate the occurrence of the sine illusion as well as the suggested transformations using data gathered from the Energy Information Administration's historical database of gas prices \citep{EIA}. This data, collected from 1995-2014, includes prices for all three grades of gasoline as well as two chemical formulations which are sold in different geographic areas \citep[for more information, see ][]{EIA-reformulated}. Figure \ref{fig:gasprices} shows the gas prices over the period of interest; there is a clear nonlinear trend to the data, with several dramatic price changes. This trend masks the increasing variance shown in Figure \ref{fig:gasvariance}.

\begin{SCfigure}\centering
\caption{Standard deviation of daily gas prices between 1995 and 2014. There is a clear increasing trend in the variance that is masked by the increasing price per gallon in Figure \ref{fig:gasprices}.\label{fig:gasvariance}}
<<gaspricevariance,echo=F,include=T, fig.width=6, fig.height=2, out.width=".6\\textwidth", dependson='gasprices'>>=
ggplot( data=subset(monthly, year(month)>=1995) ) + geom_line(aes(month, sd)) + ylab("Standard deviation in daily gas prices (USD)")  + xlab("Time")+ theme_bw() #+ ggtitle("Standard Deviation of Gas Prices")
@
\end{SCfigure}
...
ggsave(file="figure/gas-x-corrected.pdf", width=8, height=4)
...
ggplot(data=gas.cor.y) + 
  ...
  theme_bw() + geom_point(aes(x=1995, y=0), size=0.1)
ggsave(file="figure/gas-y-corrected.pdf", width=8, height=4)

@

\paragraph{Transformation in $X$}
We applied the $X$ transformation to the gas prices data  using the unweighted transformation and the transformation with $w=0.36$. This value was chosen as it is approximately the midpoint of the range of acceptable values determined experimentally. The corrected and uncorrected data with fitted cubic splines are shown in Figure \ref{fig:gasprices-x-correct}.
\begin{figure}[hbtp]
\centering
\includegraphics[keepaspectratio=TRUE,width=.95\linewidth]{figure/gas-x-corrected}
\caption{Gas price data corrected using the $X$ transformation with $w=1$ and with $w=0.36$. At $w=1$, the transformation is severe, but it is clear that the variance is lower between 1995 and 2000 than it is between 2009 and 2014. When $w=0.36$, the transformation is much less noticable but results in a near-constant absolute slope of the fitted line.}
...
% The full, unweighted transformation does distort the $X$ axis such that data collected on monthly intervals no longer appears to be uniformly distributed in $X$. The transformation using $w=0.36$ is more moderate and still largely breaks the illusion, and as such may be preferrable to the full $w=1$ transformation. 

\paragraph{Transformation of $Y$}
We also applied the Y transformation to the gas prices data using the unweighted transformation and the weighted transformation with $w=0.40$. This value was chosen as it is approximately the midpoint of the range of acceptable values determined experimentally. The corrected and uncorrected data with fitted cubic splines are shown in Figure \ref{fig:gasprices-y-correct}. 
\begin{figure}[hbtp]
\centering
\includegraphics[keepaspectratio=TRUE,width=.95\linewidth]{figure/gas-y-corrected}
\caption{Gas price data corrected using the $Y$ transformation with $w=1$ and with $w=0.40$. At $w=1$, the transformation is severe, but it is clear that the variance is nearly constant between 1995 and 2000 and then begins to increase with the price of gas, becoming much larger between 2010 and 2014. When $w=0.40$, the transformation is much less noticable, and the resulting $y$-axis scale is much more similar to the uncorrected data. }
...
% \caption{USHCN temperature data from 2008-2013 corrected using the $X$ transformation with $w=1$ and with $w=0.36$. At $w=1$, the transformation is severe, but it is clear that the variance decreases in the summer months and increases in the winter. When $w=0.36$, the transformation is much less noticable but results in a near-constant absolute slope of the fitted line. The weighted transformation removes the presence of the sine illusion without excessively distorting the underlying function shape.}
