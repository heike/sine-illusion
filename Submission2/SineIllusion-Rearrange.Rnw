...
\centering
\vspace{-.3in}
\caption{\label{fig:OrthogonalWidth} \newdo{Should we lightly shade the area between the two hashed lines in grey?} The sine illusion with lines orthogonal to the tangent line at $f(x)$. The perception that the vertical length changes with $f(x)$ corresponds to changes in actual orthogonal width due to the change in the visual (plotted) secant angle. The strength of the perceptual effect depends in part on the aspect ratio of the graph, as shown in the second image.
...
\newdo{ The `many situations' need to be addressed too - we're thinking of publishing situations - for exploring we can possibly have as many pictures as we want, but the problem is that we need to have the idea that we need more than one picture. Let's stress the awareness issue here. }
...
\caption{Corrected gas price data using  $Y$-transformations with $w=1$ and  $w=0.40$. \newdo{The y axis are not showing the right values - for the fully corrected values it would suggest that there were negative prices involved. I'm not sure how to fix this, though, except for taking off horizontal lines and any labelling.}}
