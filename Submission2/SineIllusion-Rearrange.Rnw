...
ggplot()  + geom_jitter(aes(Time, value/100), data=subset(ushcn, Year>1995), size=1, shape=I(1), colour=I("grey30"), alpha=I(.90)) +
  geom_line(aes(time, avg/100), data=subset(monthly, year(time)>1995), colour="black", size=1.5) + 
...
\caption{Raw monthly maximum temperatures (in Celsius) between 1996 and 2012 at 380 stations across the US (as indicated on the map) with similar elevation and latitude. The blue line shows fitted temperatures from a baseline model. The plot suggests that the model fails to capture the more extreme values in summer and winter. \label{fig:raw-temp}}
\end{figure}
