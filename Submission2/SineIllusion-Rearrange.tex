...
{\centering \includegraphics[width=\linewidth]{figure/fig-UncorRresults} 
...
{\centering \includegraphics[width=\linewidth]{figure/fig-LinearResults} 
...
{\centering \includegraphics[width=\linewidth]{figure/fig-CtsAdjustedResults} 
...
%Once outliers were removed, each participant completed an average of  round(trials.per.participant, 2) trials. As there were only sum(!trial.sum$incl.startwt & trial.sum$incl.trials & !trial.sum$endwt.outlier) trials with starting weight outside of [0,1] which met the other inclusion criteria, those trials were eliminated from the analysis to prevent high-influence points from adversely influencing the model. 
...
{\centering \includegraphics[width=.6\textwidth]{figure/fig-gasprices-pictures} 
...
{\centering \includegraphics[width=.5\textwidth]{figure/fig-gaspricevariance} 
...
{\centering \includegraphics[width=.5\linewidth]{figure/fig-meanDists} 
...
According to this analysis, the optimum transformation value for $x$ is 0.35, and the optimum transformation value for $y$ is 0.45. Figure \ref{fig:psycho} shows the estimates and 95\% Wald intervals for the mean, as well as estimated density of participant-level responses. %In addition, only 6 user averages were outside of the range of (0,1) for either trial. 
...
%quantile(probs=c(0.025, 0.5, 0.975), as.vector(model.mcmc@sigma))}
\end{table}
...
% 
% Figures \ref{fig:raw-temp} and \ref{fig:boxplot-temp} together suggest that winter months have the most variability, while spring, summer, and fall have similar variability. In fact, the variance of fall months is greatest (round(season.variation[3,3])), followed by winter (round(season.variation[4,3])), spring (round(season.variation[1,3])) and finally summer (round(season.variation[2,3])). In fact, the winter variability is round(with(season.variation, StdDev[4]/StdDev[2]), 1) times that of the summer variation. The sine illusion hides these differences in variance effectively, and additionally minimizes the size of the variance in spring and fall. The problem here is not the graphic itself, as Figure \ref{fig:raw-temp} is true to the data, but rather that our brains cannot translate the graph into quantitative measurements with sufficient accuracy.
% 
