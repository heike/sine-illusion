...
Another example of this phenomenon is shown in Figure \ref{robbins-graph}. The line-width illusion is also present in the graphs of $e^x$ and $e^x + 1$, as the lines appear to be separated by a much smaller distance at $x=2.5$ than at $x=0$.
\begin{figure}[htbp]
\centering
