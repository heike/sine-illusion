...
qplot(data=datasub, x=jitter(Tmax), y=Ozone, geom="point", colour=I("grey"), alpha=I(.75), xlab="Temperature (F)", ylab="8-hour Average Ozone Concentration (ppm)") + geom_line(aes(x=Tmax, y=fit)) + theme_bw()
qplot(data=datasub, x=jitter(Tmax), y=resid, geom="point", colour=I("grey"), alpha=I(.75), xlab="Temperature (F)", ylab="Residual Ozone Concentration (ppm)") + geom_line(aes(x=Tmax, y=0*resid)) + theme_bw()

@

\done{We need a relevant example as a hook - then explain to people what trap they fell in.}

Figure \ref{example1} shows daily measurements of 8-hour average ozone concentration and temperature at several sites in Houston \citep{epa}, for days in 2011 with temperature above $45^o$ F and dew point less than $60^o$ F (as ozone concentration is related to temperature and humidity) with a loess smooth line for reference. Ozone concentrations were gathered from \Sexpr{nsite} locations in the Houston area \citep{epa}, and weather data is aggregated from the NCDC \citep{noaa} site at Hobby International Airport, which is located in the center of the Houston area. 

\begin{figure}[h!tbp]\centering
\caption{Ozone and Temperature in Houston, 2011. What seems to be the trend in overall variance? Is the variance trend in (a) consistent with the residual plot in (b)?}
...
\begin{subfigure}[b]{.45\linewidth}
  ...
  \caption{\small Outliers removed}
  \label{example2}
\end{subfigure}
\label{exampleFull1}
\end{figure}
The sine illusion (also known in graphical literature as the line width illusion) is a frequent occurrence in statistical analysis, and graphical displays should be thoughtfully considered to minimize its effect visually and acknowledge its influence. Figure \ref{original} shows the classical sine illusion with the parameters which determine the illusionn's strength.  
...
\end{figure}
The first documentation we could find of this illusion, \cite{day:1991} documented the illusion in the context of sinusoidal vertical lines. More recently the illusions have been documented on non-sinusoidal curves \todo{add Citation for hammock plots}, but the illusions seem to be the same.  
...
\caption{A replication of Figure 2.16 in \cite{robbins:2005}. The line width illusion seems to be related to increasing slope - as the slope increases, the apparent distance between the two lines decreases, even though the lines are separated by a constant distance.}
\label{robbins-graph}
\end{figure}
...
\subsubsection{Trigonometry Method}
