...
\caption{The original Sine Illusion, which consists of evenly spaced vertical lines centered around the line $y=\sin(x)$. The peak and trough of the curve appear to be composed of longer lines than the other regions.}\label{fig:original}
...
\caption{A replication of Figure 2.16 in \cite{robbins:2005}. Lines $y=e^x$ and $y=e^x+1$ appear  to converge as $x$ increases due to the line width illusion.}
