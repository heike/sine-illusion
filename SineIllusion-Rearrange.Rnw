...
w01 <- wopts[(wopts<=.75 & wopts>=.25))]
qplot(wopts, rep(1, length=length(wopts)), geom="point") + 
  geom_vline(xintercept =c(0,1), colour="grey80") + 
  theme_bw() + 
  ylab("") + ylim(c(0.8, 1.2)) + 
  xlab("Starting weight") + xlim(c(-.2,1.2)) + 
  geom_point(aes(x=w01, y=rep(1, length(w01))), pch=1, size=4) + 
  theme(axis.ticks.y=element_blank(), axis.text.y=element_blank())  + 
  geom_point(aes(x=c(0,1), y=rep(1, 2)), pch=1, size=6) + 
  theme(axis.ticks.y=element_blank(), axis.text.y=element_blank())
@
...
The study is set up as a fractional factorial design of correction type ($x$ or $y$ correction) and starting weight $w_0$. Each participant is asked to evaluate a total of twelve situations, six of each correction type, though they are allowed to complete up to 30 trials if they wish. For the first six starting weights $w_0$ values were chosen with equal probability from the set of numbers $\{0, 1, c\}$, where $c \sim U(.25, .75)$, while the second set of starting weights is randomly sampled from a distribution centered at 0.6 with mass on discrete values in the interval $[0,1]$, shown in figure \ref{fig:weightdist}. The distribution was chosen to have a higher coverage density around 0.6 after a pilot study indicated a preference for that value. Using a distribution with a wide range of coverage allows us to fully explore the space of plausible $w$ while focusing on weights near the center of the $(0,1)$ interval. Table \ref{factortable} summarizes the factors under investigation in the experiment.
}
