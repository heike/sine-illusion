...
\newdo{Main message: Sine illusion is there, and the correction provides evidence that the illusion is there. }
...
\cite{day:1991} suggest that the sine illusion occurs due to misapplication of perceptual experience with the three-dimensional world to a two-dimensional ``artificial" display of data. 
...
% While it is typically a bad idea to use three-dimensional representations when two dimensions are adequate to display the data \citep{tufte, cleveland:1984}, it might present a viable avenue to counteract this  illusion.
...
qplot(x=x, y=y, geom="line", data=dframe, colour=I("grey50")) + theme_bw() + 
  ...
  theme(legend.key.width = unit(3, "line"), plot.margin = unit(c(0,0,0,0), "cm"), legend.position=c(0, -1)) 
@
\begin{SCfigure}
\centering
...
@
<<results-analysis1,echo=FALSE, include=FALSE, fig.width=8, fig.height=5, dependson='results-analysis'>>=
ggplot() + theme_bw() + facet_grid(type~., scales="free") + ylim(c(0, 27)) +
  ...
  theme(legend.position="bottom", plot.margin=unit(c(0,0,0,0), unit="cm")) + geom_point(aes(x=ests, y=20), data=ests) +
  xlim(c(-.2, 1.2)) +
#   geom_point(data=ints.all, aes(x=xmax, y=0, size=factor("Range")), shape=NA, colour="grey75") +
#   guides(colour=guide_legend(order=1), fill=guide_legend(order=1), size=guide_legend("Range of\nacceptable weights", override.aes=list(shape=15, size = 10), order=3)) +
  geom_text(data=ints, aes(x=x, y=22.5, label=label), parse=TRUE) + 
  geom_text(data=ints.all, colour="grey30", aes(x=x, y=15.5, label="Range of\nacceptable weights")) + theme(plot.margin=unit(c(0,0,0,0), unit="cm"))
...
\begin{itemize}
\item Participant did not interact with the applet: we required a participant to use the adjustment at least once in order to include data for this trial (592 trials removed).
...
\caption{Estimated density of participant-level means using the standard method of limits analysis described in \citet{goldstein}. The overall means are both near 0.4, however, there is quite a bit of user-level variability.}\label{fig:psycho}
<<meanDists,echo=FALSE,out.width='.6\\linewidth', dependson='results-analysis', fig.width=8, fig.height=4, message=FALSE, warning=FALSE>>=
...
@
\end{SCfigure}
The psychophysics model is based on  weighted averages (by adjustment type) of all trials with starting weights $w_0 = 0$ and 1:
...
These results suggest that some transformation is needed. At the same time, we see that a complete transformation is not necessary to counteract the illusion. For  more precise bounds on the range of acceptable transformation weights we will use a linear model that  incorporates starting points other than 0 and 1, while simultaneously allowing for user-specific variability. % as well as data from users that completed a sufficient number of trials but did not complete a trial starting at one of the endpoints. 
...
\end{figure}
Figure \ref{fig:MixedModelResults} gives an overview of the relationship between  starting weights and  user-submitted preferred weight values. Higher starting weights are associated with higher user-submitted values, and lower starting weights are associated with lower user-submitted values. 
...
season.variation <- ddply(ushcn, .(Season), summarize, Variance=var(value), StdDev=sd(value), Mean=mean(value))
@
...
\caption{Raw monthly maximum temperatures (in Celsius) between 2008 and 2013 at 380 stations across the US (as indicated on the map) with similar elevation and latitude. The line shows fitted temperatures from a baseline model. The plot suggests that the model fails to capture the more extreme values in summer and winter. \label{fig:raw-temp}}
\end{figure}
...
Figures \ref{fig:raw-temp} and \ref{fig:boxplot-temp} together suggest that winter months have the most variability, while spring, summer, and fall have similar variability. In fact, the variance of fall months is greatest (\Sexpr{round(season.variation[3,2])}), followed by winter (\Sexpr{round(season.variation[4,2])}), spring (\Sexpr{round(season.variation[1,2])}) and finally summer (\Sexpr{round(season.variation[2,2])}). In fact, the winter variability is \Sexpr{round(with(season.variation, Variance[4]/Variance[2]), 1)} times that of the summer variation. The sine illusion hides these differences in variance effectively, and additionally minimizes the size of the variance in spring and fall. The problem here is not the graphic itself, as Figure \ref{fig:raw-temp} is true to the data, but rather that our brains cannot translate the graph into quantitative measurements with sufficient accuracy.
...
ushcn.cor <- melt(ushcn.cor, id.vars=c(1:7, 10))
...
ushcn.cor2 <- melt(ushcn.cor2, id.vars=c(1:7, 10))
...
ggplot(data=ushcn.cor.x) + 
  ...
  theme_bw()
ggsave(file="figure/ushcn-x-corrected.pdf", width=10, height=8)
...
ushcn.cor <- melt(ushcn.cor, id.vars=c(1:6, 8, 10))
...
ushcn.cor2 <- melt(ushcn.cor2, id.vars=c(1:6, 8, 10))
...
ggplot(aes(x=time, y=value, group=variable), data=ushcn.cor.y) + 
  ...
  theme_bw()
ggsave(file="figure/ushcn-y-corrected.pdf", width=10, height=8)
@
...
\caption{USHCN temperature data from 2008-2013 corrected using the $X$ transformation with $w=1$ and with $w=0.36$. At $w=1$, the transformation is severe, but it is clear that the variance decreases in the summer months and increases in the winter. When $w=0.36$, the transformation is much less noticable but results in a near-constant absolute slope of the fitted line, removing the presence of the sine illusion without excessively distorting the underlying function shape.}
