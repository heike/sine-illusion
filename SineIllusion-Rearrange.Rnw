...
\newdo{The gridlines are off - can you please double check?; the boxes are also gone missing;
could you try to make the pictures a bit bigger - plot.margins of 0 should do the trick}}
...
\newdo{We need to go through the paper and get rid of the 95\% HDP intervals ... we are not doing MCMC anymore, so this needs to be adjusted}
...
@
\newdo{Could you make this a table of means with 95\% CI?\\ What kind of CI? Just estimated from the density? Or do you want me to use a Wald interval and assume normality?\\
I guess the psychophysics guys would estimate standard deviation from the sample and put it in a Wald CI ...}
...
ushcn.cor <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="y", f=f, fprime=fprime, f2prime=f2prime, weight=1, ratio=1/50)
...
ushcn.cor2 <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="y", f=f, fprime=fprime, f2prime=f2prime, weight=.40, ratio=1/50)
...
\end{figure}
\newdo{The $y$ correction is unnoticeable because the aspect ratio is sooo weird in this plot. Our correction factor is 1/90, because we have 4 years of data and 60 degrees temp difference, spread over a plot that is 2in high by 12 in wide. For demonstration purposes, I've tweaked it to be 1/50 instead, which shows some slight differences.}
%\clearpage
\section{Conclusion}
