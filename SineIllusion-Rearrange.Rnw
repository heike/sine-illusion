...
df <- createSine(200, len=1, f, fprime)[,c(1, 4, 7:9)]
df2 <- ddply(df, .(x), transform, ysamp = rnorm(30, y, ellx/2))
qplot(data=df2, x=x, y=ysamp, geom="point") + xlab("x") + ylab("y") + coord_equal(ratio=1) + theme_bw() +ylim(c(-4,4))         
...
f2prime <- function(x) 2*sin(4*x/pi)
df <- createSine(200, len=1, f, fprime)[,c(1, 4, 7:9)]
df2 <- ddply(df, .(x), transform, ysamp = rnorm(30, y, mean(df$ellx/2)))
qplot(data=df2, x=x, y=ysamp, geom="point") + xlab("x") + ylab("y") + coord_equal(ratio=1) + theme_bw() +ylim(c(-4,4))     
qplot(data=df2, x=x, y=ysamp-y, geom="point") + xlab("x") + ylab("y") + coord_equal(ratio=1) + theme_bw() +ylim(c(-3,3))  
@
