...
\end{figure}
The first documentation we could find of this illusion, \cite{day:1991} documented the illusion in the context of sinusoidal vertical lines. More recently the illusions have been documented on non-sinusoidal curves \citep{schonlau:2003, marie}, but the illusions seem to be the same.  


\subsection{Statistical Graphics and the Sine Illusion}\label{statisticalgraphics}
% While the illusion presented above is termed the sine illusion, it holds for non-sinusoidal curves which have dramatic changes in the slope of the lines over time as well.
...
An interesting facet of the sine illusion is that when the graph is examined as if it were a two-dimensional projection of a three-dimensional figure, the illusion disappears and the line widths seem once more to be constant. In addition, if the lines are considered without the surrounding context, judgments comparing two lines seem to be relatively accurate
% , particularly if the graph also includes horizontal reference lines, as in Figure \ref{original-grid}. 
\done{this statement is in direct contradiction of the figure caption.}\comment{Not really, as the important clause is that the lines are considered ``without the surrounding context", but I've removed the reference.}
...
\done{I'm not quite sure where this paragraph belongs, but it should probably not be a subsection of its own - it reads rather like a segue in or out of a section ...}
\paragraph{Application: Simulation}
Sections \ref{perceptualexplanations} and \ref{statisticalgraphics} explore the perceptual and statistical literature examining this illusion. The ``line width illusion" and the ``sine illusion" are the same phenomenon, as explicitly demonstrated in Figure \ref{simulation1}. 
\begin{figure}[h!tbp]\centering
\begin{subfigure}[b]{.31\linewidth}
  \centering
<<simulation1Sine, echo=FALSE, fig.width=4, fig.height=4>>=
...
<<simulation2SineTrend, echo=FALSE, fig.width=4, fig.height=4>>=
x <- rep(seq(0,2*pi*2, length=120), each=100)
...
\end{subfigure}
\begin{subfigure}[b]{.31\linewidth}\centering
<<simulation3Trend, echo=FALSE, fig.width=4, fig.height=4>>=
x <- rep(seq(0,2*pi*2, length=120), each=100)
...
@
  ...
  \label{simulation3}
\end{subfigure}
\caption{(a) shows seasonality without any underlying trend, (b) shows seasonality superimposed on a quadratic trend, and (c) shows a quadratic trend without seasonality. Though all three sets of simulated data have constant variance, none of the variances appear constant due to the sine illusion.}
\end{figure}

When we judge the variability of data, we must overcome both the visual bias due to the line width illusion and the visual bias due to the sine illusion. As it turns out, these two illusions are incredibly difficult to overcome, even when they are obviously present. What follows is a compilation of various methods to correct for or mitigate the effect of these two illusions.
...
#             data=dframe.1, parse=TRUE, hjust=1, vjust=.5, size=2.5) + 
  geom_text(aes(label=paste("theta", "%~~%", round(vangle/pi*180), "^o", sep=""),
                x=xstart/2+x/2-sign(vangle)*.1, y=y-sign(vangle)*.64+.04), 
            data=dframe.1, parse=TRUE, hjust=.75, vjust=.5, size=3) + 
...
% \caption{Alternate Figure Option -- The perceived length of the lines is a function of the extant width, that is, the width of the secant line at a given point. The secant lines are of varying lengths (.4 to 1), suggesting that to counter the illusion, the line lengths must be altered such that the extant widths are perceived to be equal.}\label{altOrthogonalWidth}
% \end{figure}
...
One approach to breaking the sine illusion is to present the trend and variance separately, as shown in Figure \ref{trendremoval} and recommended by \cite{wainer:2000}. A Loess smooth was used to estimate the trendline, and this estimate was subtracted from the line center to produce separate graphs of the trend and the variance. While this does largely remove the sine illusion, it also makes it somewhat more difficult to parse the overall data, as judgments must be made by combining two graphs or by mentally re-composing the original graph (at which point, the sine-illusion becomes a factor). Ideally, it would be possible to tell the whole story with a single graph, to minimize cognitive demands. While this example is relatively simple, it would be much harder to create a de-trended version of Figure \ref{playfair-debt} that could be easily assessed to determine both trend and change in the size of the trade imbalance over time. 
...
\newdo{introduce $f$, mention that one assumption has to be that $f^\prime$ exists.}

That is, if the $x$-axis spans the interval $[a,b)$, the transformation of the $x$ axis is 
$$T(x) = (b-a)\left(\int_a^x |f^\prime(z)| dz\right)/\left(\int_a^b|f^\prime(z)| dz\right)$$ 

\newdo{the goal is to make $|f^\prime(x)|$ a constant, right? Does the above transformation achieve this? That's something that you still need to show.}

This transformation can be considered as a reparameterization of the $x$ axis in terms of the slope. As the sine illusion depends on changing slope in the overall trend, it makes sense to reparameterize the axis in terms of the slope so that it is possible to see the other information contained in the graphic.This transformation is shown in Figure \ref{xtrans1}.
...
<<xtransform, echo=FALSE, fig.width=4, fig.height=2>>=
...
\end{subfigure}
\begin{subfigure}[b]{.48\linewidth}\centering
<<xtrans-mid,echo=FALSE, fig.width=4, fig.height=2>>=
...
\begin{subfigure}[b]{.48\linewidth}\centering
<<xtrans-mid2,echo=FALSE, fig.width=4, fig.height=2>>=
...
\end{subfigure}
\begin{subfigure}[b]{.48\linewidth}\centering
<<xtrans-mid3,echo=FALSE, fig.width=4, fig.height=2>>=
...
\caption{$X$ axis transformations. Dots at the bottom of the graph show the transformation's effect on equally spaced points along the $x$-axis. \newdo{I love it!!!!!}  In (a), adjusting the $x$-spacing of the lines mitigates the sine illusion by changing the extant width so that it is uniform in $x$. (b) - (d) shrink the transformation in (a) towards successively more uniform spacings in $x$ while still breaking the effects of the illusion.}
\label{xtrans}
\end{figure}
\clearpage
...
\subsubsection{Hammock Correction}\label{hammock}
This approach aims to extend the orthogonal width, which onlookers generally evaluate,  to the amount of the vertical width, which we want to represent \citep{schonlau:2003}. 

\newdo{Careful!!!!  The Trig method has nothing to do with what Matthias Schonlau was doing! It's aseparate approach and we need to highlight its pros and cons separately}
\comment{I'm confused - the Trig section is just the previously named hammock section, but it was renamed because you didn't want to over-emphasize the hammock plots?}
...
ggplot(data=df, aes(x=xstart, xend=xend, y=ystart, yend=yend)) + theme_bw() + coord_fixed(ratio=1) + 
  ...
  geom_text(aes(x=dfsec$sec.xend, y=dfsec$sec.yend-.05, 
                label="slope: -1/paste(f, \"'\", (x))"), parse=TRUE, hjust=.75, vjust=1) +
...
\caption{Geometry of the trigonometry correction approach.}
\end{figure}
\comment{The picture is wrong here, but I can't figure out how the extant width in the trig method was actually calculated. It seems like it should be like this, but either we're assuming a right angle and shouldn't be (use law of cosines instead) or I'm being an idiot, but the picture looks exactly like that for linear fix, and I know it isn't the same method!}
...
$\left[\cos(\atan{\left|f^\prime(x)\right|})\right]^{-1}$ to make the extant width appear as long as $\ell$ in the original graph. 
...
\caption{Trigonometry correction}
\end{subfigure}
...
to obtain the lower endpoint $(x_l, y_l)$. This processis illustrated in Figure \ref{GeneralCorrection}. With these two endpoints, we can correct separately for the top and bottom portions of the line, as the extant width is not symmetric when reflected across the original function f(x). 
...
dfslope <- data.frame(xstart=df$x[a]+.5, xend=df$x[a]-.9, ystart=fprime(df$x[a])*.5+df$y[a], yend=-.9*fprime(df$x[a])+df$y[a])
dfslope2 <- data.frame(xstart=df$x[a]+.5, xend=df$x[a]-.3, ystart=fprime(df$x[a])*.5+df$y[a]+.5, yend=-.3*fprime(df$x[a])+df$y[a]+.5)
qplot(data=df.real, x=x, y=y, geom="line") + theme_bw() +
...
#                 label="slope: -1/paste(f, \"'\", (x))"), parse=TRUE, hjust=.75, vjust=1) +
  ...
  geom_text(data=dfsec, aes(x=sec.xstart+.02, y=sec.ystart+.04, label="paste(group('(',list(x[u],y[u]),')'))"), parse=TRUE, hjust=0) + 
  geom_text(data=dfsec, aes(x=sec.xend, y=sec.yend-.06, label="paste(group('(',list(x[l],y[l]),')'))"), parse=TRUE, hjust=.9) +   
  geom_text(data=dfslope, aes(x=xend+.1, y=yend+.05, label="slope: paste(f, \"'\", (x))"), parse=TRUE, hjust=0, vjust=1) +
  coord_equal(ratio=1)
...
dfslope <- data.frame(xstart=df$x[a]+.5, xend=df$x[a]-.9, ystart=fprime(df$x[a])*.5+df$y[a], yend=-.9*fprime(df$x[a])+df$y[a])
dfslope2 <- data.frame(xstart=df$x[a]+.5, xend=df$x[a]-.3, ystart=fprime(df$x[a])*.5+df$y[a]+.5, yend=-.3*fprime(df$x[a])+df$y[a]+.5)
qplot(data=df.real, x=x, y=y, geom="line") + theme_bw() + coord_equal(ratio=1) +
  geom_line(data=df.real,aes(y=yupper), linetype=3) + 
  geom_line(data=df.real,aes(y=ylower), linetype=4) + 
  geom_segment(data=df, aes(x=xstart, y=ystart, xend=xend, yend=yend), colour="grey50") + 
  geom_segment(data=df[a,], aes(x=xstart, y=ystart, xend=xend, yend=yend), colour="black", size=1.5)+
...
#                 label="slope: -1/paste(f, \"'\", (x))"), parse=TRUE, hjust=.75, vjust=1) +
  geom_segment(data=dfslope, aes(x=xstart, y=ystart, xend=xend, yend=yend), linetype=2) + 
  geom_text(data=dfsec, aes(x=sec.xstart+.44, y=sec.ystart, label="paste(group('(',list(x[u],y[u]),')'))"), parse=TRUE, hjust=1, vjust=.25) + 
  geom_text(data=dfsec, aes(x=sec.xend-.34, y=sec.yend, label="paste(group('(',list(x[l],y[l]),')'))"), parse=TRUE, hjust=0, vjust=.25) 
...
#                 label="slope: -1/paste(f, \"'\", (x))"), parse=TRUE, hjust=.75, vjust=1) +
  geom_segment(data=dfslope, aes(x=xstart, y=ystart, xend=xend, yend=yend), linetype=2) + 
  geom_text(data=dfsec, aes(x=sec.xstart+.4, y=sec.ystart, label="paste(group('(',list(x[u],y[u]),')'))"), parse=TRUE, hjust=1, vjust=.25) + 
  geom_text(data=dfsec, aes(x=sec.xend-.34, y=sec.yend-.075, label="paste(group('(',list(x[l],y[l]),')'))"), parse=TRUE, hjust=0, vjust=.25) 
@
\begin{figure}[h!tbp]\centering
...
\caption{General Correction}\label{GeneralCorrection}
\end{subfigure}
\begin{subfigure}[b]{.32\linewidth}\centering
...
\end{subfigure}
\caption{General correction approach. (a) may require numerical optimization to obtain an exact solution, (b) uses a first-order taylor series approximation to $f(x)$ and (c) uses a second-order taylor series approximation to $f(x)$. The intersection of the function $f(x) \pm \ell/2$ and the secant line,  $(x_u, y_u), (x_l, y_l)$ must be obtained to determine the necessary correction factor.}
\end{figure}
...
1/\left(\sqrt{1 + f^\prime(x)^2}\right)
\end{equation}
...
As shown in Figure \ref{quadratic-GeneralCorrection}, solving the system of equations provides endpoints $(x_l, y_l)$ and $(x_u, y_u)$ which can then be used to compute the extant width of each half-segment $\ell_\perp$. Unlike the linear solution, the half-segments here are not of the same length, as shown in Figure \ref{HalfSegLength}, and thus there are separate correction factors for each half-segment. The correction factors are $(\ell/2)/(\ell_{\perp_u})$ and $(\ell/2)/(\ell_{\perp_l})$ respectively. 
