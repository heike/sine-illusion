...
\newtext{The increasing variance with temperature is camouflaged in the Figure \ref{example1} due to the changes in slope, but is very obvious in the residual plot in Figure \ref{example2} once the trend has been removed.}
...
\]
Since we can write $|f(x_j) - f(x_{j-1})| = \int_{x_{j-1}}^{x_j} |f^\prime(z)|dz$ we get a more concise expression for the transformation as
...
In practice we can see that the illusion can be broken by a much less severe transformation of the $x$ values. 
By shrinking the transformation in Figure \ref{xtrans1} towards the original $x$ values by a factor of $w \in (0,1)$, the illusion remains broken but the data is reflecting the  original function of $f(x)$ much more closely. We use the following equation for this: 
% \begin{equation}\label{eqn.xtrans.weighted}
% (f \circ T_w)(x) = \frac{w-1}{w} x + \frac{(b-a)}{w}\left(\int_a^x |f^\prime(z)| dz\right)/\left(\int_a^b|f^\prime(z) dz|\right)
% \end{equation}
\begin{equation}\label{eqn.xtrans.weighted}
(f \circ T_w)(x) = w \cdot x + (1-w) \cdot (f \circ T)(x)
\end{equation}

Figures \ref{xtrans2} - \ref{xtrans4} show the effect of different shrinkage coefficients $w$. The extent to which this weighting holds varies with the aspect ratio of the chart and the shape of the function. It also depends on the audience's experience with the sine illusion, resulting in very subjective choices of an
