...
\done{define ranges of i and j - how many participants did we have after the cleaning? - we should state that above.\\ It is stated above. (2nd bullet point on the exclusion criteria)}
\newtext{Let $W_{ij}$ denote the final adjustment to weight by participant $i$, $ 1 \le i \le \Sexpr{nparticipants}$, on trial $j$, $1 \le j \le n_i$. }
...
We use MCMC simulation to generate responses for each correction type and each participant from the model, which we use to both  create user-level densities as well as population-based posterior densities for the model parameters that allow us to determine credibility intervals. 


The variability of the random effects for each trial type is similar; but the model benefits significantly from allowing separate random effects for individual's variability by correction type ( \Sexpr{library(lme4); summary(model2)@REmat[1,3]} and \Sexpr{summary(model2)@REmat[2,3]} for $Y$ and $X$ transformations, respectively, as opposed to \Sexpr{ summary(model)@REmat[1,3]} for the overall variability). The interaction between starting weight and trial type was not significant, however, and was thus removed from the model ($p$-value = \Sexpr{anova(model2, model3)$`Pr(>Chisq)`[2]}).
...
<<<<<<< HEAD
<<model-sim, echo=FALSE, include=FALSE, dependson='results-analysis'>>=
# library(languageR) # gets p-values for lmer models
# library(xtable)
# res <- pvals.fnc(model, nsim=5000, ndigits=4, withMCMC=TRUE)
...
<<modelres, echo=FALSE, results='hide', dependson='model-sim'>>=
require(xtable)
