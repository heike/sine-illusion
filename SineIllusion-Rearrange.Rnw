...
asp <- ((2)/100)/(10/17) # image vs coordinate aspect ratio 
startyear <- 1995
gridlines <- seq(1996, 2013, .625)
...
ggplot(data=ushcn.cor.x) + 
  ...
  facet_grid(variable~.) +
  geom_line(data=ushcn.cor.x, aes(x=value, y=temp.fit), colour="red", size=1.5) + 
...
ggplot(aes(x=time, y=value, group=variable), data=ushcn.cor.y) + 
  geom_jitter(size=1.5, alpha=.75)+ 
  facet_grid(variable~.) +
  geom_line(aes(y=temp.fit), colour="red", size=1.5) + 
  xlab("Year") +  
  ylab("Temperature (in Celsius)") + 
  theme_bw()
