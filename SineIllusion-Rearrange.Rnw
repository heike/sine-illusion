...
The orthogonal width is given as the line segment between endpoints $(x_1, f_1(x_1))$ and $(x_2, f_2(x_2))$, where $f_1$ and $f_2$ denote the vertical shifts of function $f$ by $-\ell/2$ and $\ell/2$, respectively (that is, $\ell$ is the overall line length, and $\ell/2$ is the length of the upper or lower segment).
...
In the following two sections we are going to use Taylor approximations of first and second order to find approximate solutions to the end points. These approximations are shown in figure \ref{fig:linear.quadratic}.
...
The starting weights were distributed as follows: Each user was given a trial of each type starting at 0 and 1, and then the remaining four trials of each type had starting weights chosen with equal probability from $c \in \left\{0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.525, \right.$ $\left. 0.55, 0.575, 0.6, 0.625, 0.65, 0.675, 0.7, 0.725, 0.75\right\}$. The possible starting values are shown in figure \ref{fig:weightdist}; possible user-submitted results are shown as solid circles, while starting weights are represented as unfilled circles. This  distribution of starting weights was chosen to have a higher coverage density around 0.6 after a pilot study indicated a preference for that value. Using a distribution with a wide coverage allows us to fully explore the space of plausible weight $w$ while focusing on the $(0,1)$ interval and enabling precise estimation of the optimal $w$ in the region indicated by the pilot study. %Table \ref{factortable} summarizes the factors under investigation in the experiment.
...
\caption{Overview of possible starting weights. Weight values are discrete, but staggered so as to provide fine-grained adjustments around 0.6 and more coarse discriminatory information outside of $(0,1)$. Hollow circles indicate choices of starting weights for the first 6 trials, filled circles represent weights available for users to submit. All starting weights (with the exception of 0 and 1) are selected with equal probability.}\label{fig:weightdist}
\end{figure}
...
\caption{Simulation results from the fitted model, facetted by correction type. Fixed effects results are shown as histograms; the red values display the results when starting from an uncorrected plot and are concentrated around $w=\Sexpr{round(mean(subset(ints, type=="X Transformation")$x), 2)}$; the blue values represent user-chosen weights when starting from a fully corrected plot and are concentrated around $w=\Sexpr{round(mean(subset(ints, type=="Y Transformation")$x), 2)}$. Additionally, 95\% bootstrap intervals are shown as horizontal line segments above the histograms; these intervals are for the lower and upper bounds of the ``preferred weight interval" tested in the experiment. User-level density curves show the individual variability around  fixed effects $\alpha_*$ and $\alpha_*+\beta$.}\label{fig:MixedModelResults}
\end{figure}
Figure \ref{fig:MixedModelResults} shows density plots which demonstrate the relationship between the starting weight value and the user-submitted ``optimal'' weight value. Higher starting weights are associated with higher user-submitted values, and lower starting weights are associated with lower user-submitted values. As users had to increment or decrement the weight value, this suggests that there is a range of acceptable values, and as users entered that range, they proceeded to the next trial. 
...
The fitted model parameters are shown in tables \ref{fixedeffectsresults} and \ref{randomeffectsresults}. Each transformation type has a lower bound (corresponding to starting at $w=0$) and an upper bound (corresponding to starting at $w=1$); these values are derived from the linear model parameters, where the lower bound is $\alpha_{T(i,j)}$ and the upper bound is $\alpha_{T(i,j)}+\beta X_{ij}$. The $X$ and $Y$ bounds are similar, with $X$ bounds slightly lower than $Y$ bounds; that is, the optimal weight is slightly higher for $Y$ than for $X$. In addition, Table \ref{randomeffectsresults} shows that the variability within a single individual's trials is higher than the variability within participants in both types of trials. 
...
asp <- .025 # image vs coordinate aspect ratio 
startyear <- 1995
gridlines <- seq(1996, 2013, 1/6)
...
ushcn.cor.x <- subset(ushcn.cor.x, variable!="Uncorrected" & value>=1996)
...
ggplot(data=ushcn.cor.x) + 
  geom_jitter(aes(x=value, y=temp, group=variable), size=1.5, position = position_jitter(width = 1/36), alpha=.75)+ 
  facet_grid(variable~.) +
  geom_line(data=ushcn.cor.x, aes(x=value, y=temp.fit), colour="steelblue", size=1.5) + 
...
ushcn.cor.y <- subset(ushcn.cor.y, variable!="Uncorrected" & time>=1996)
...
ggplot(aes(x=time, y=value, group=variable), data=ushcn.cor.y) + 
  geom_jitter(size=1, alpha=.75)+ 
  facet_grid(variable~.) +
  geom_line(aes(y=temp.fit), colour="steelblue", size=1.5) + 
  xlab("Year") +  
  ylab("Temperature (in Celsius)") + 
  theme_bw()
