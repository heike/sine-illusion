...
\item an additional scaling reflecting the ratio of original $y$- and $x$-ranges will result in the  $x$-range being preserved. This happens  for $c_i = \range(x)/\range(f(x)) \min{f({x_{i-1}), f(x_i))$ and $d_i = \range(x)/\range(f(x)) \max{f({x_{i-1}), f(x_i))$. 
\end{itemize}
