...
The following analysis is based on the cleaned data, consisting of \Sexpr{length(unique(lm.data$ipid))} participants with \Sexpr{nrow(lm.data)} valid trial results.
