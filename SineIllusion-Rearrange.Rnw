...
$f(x)$ with non-constant derivative $f^\prime(x)$ which may vary continuously. 

 This is still dependent on having a functional form for $f(x)$ which has derivative $f^\prime(x)$ on the range of $x$ within the data, as we will determine $\theta$ based on the slope of the secant line, $-1/f^\prime(x)$.
