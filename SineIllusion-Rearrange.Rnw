...
\begin{figure}[h!tbp]\centering
\comment{Might be better to use simulated data or something - the lines are a bit too artificial here - not really "data"}
<<trend-removal, include=FALSE>>=
...
ggplot(data=df,aes(x=x, y=y)) + geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend)) + geom_line(aes(group=type, linetype=type)) + scale_linetype_discrete("") + theme(legend.position="bottom") + coord_equal(ratio=1) + theme_bw()
qplot(data=subset(df, type=="Loess Fit"), x=xstart, xend=xend, y=ystart-y, yend=yend-y, geom="segment") + ylab("De-Trended Data") + xlab("x")+ ylim(c(-1.5, 1.5)) + coord_equal(ratio=1)+ theme_bw()
qplot(data=subset(df, type=="Loess Fit"), x=x, y=y, geom="line") + ylab("Fit Trendline") + xlab("x") + ylim(c(-1.5, 1.5)) + coord_equal(ratio=1)+ theme_bw()
@
...
\begin{align}
y & = f(x_u) + \ell/2\\
y & = \left(-1/f^\prime(x_0)\right) x_u + f(x_0) + x_0/f^\prime(x_0)
\end{align}
...
Figure \ref{LinearApprox} shows the correction factor compared with the hammock-plot style correction and the untransformed data. As with the hammock plot correction, the linear adjustment is prone to the same overcorrection distortion shown in Figure \ref{linearFixOvercorrect}
