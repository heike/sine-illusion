...
ggplot() + theme_bw() + facet_grid(type~., scales="free") + ylim(c(0, 27)) +
  ...
  theme(legend.position="bottom") + geom_point(aes(x=ests, y=20), data=ests) +
  xlim(c(-.2, 1.2)) +
...
\done{We need to go through the paper and get rid of the 95\% HDP intervals ... we are not doing MCMC anymore, so this needs to be adjusted}
...
\end{itemize}
\newtext{The following analysis is based on the cleaned data, consisting of \Sexpr{length(unique(lm.data$ipid))} participants with \Sexpr{nrow(lm.data)} valid trial results.}
...
$\beta$ can therefore be interpreted as the length of the interval of plausible weights. 
...
ushcn.cor <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="x", f=f, fprime=fprime, f2prime=f2prime, weight=1)
...
ushcn.cor2 <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="x", f=f, fprime=fprime, f2prime=f2prime, weight=.36)
...
ggplot(aes(x=value, y=temp, group=variable), data=ushcn.cor.x) + 
  geom_jitter(size=1, position = position_jitter(width = 1/36))+ 
  facet_grid(variable~.) +
  geom_line(data=ushcn.cor.x, aes(y=temp.fit), colour="blue") + 
  xlab("Year") +  
  ylab("Temperature (in Celsius)") + 
  theme_bw()
ggsave(file="figure/ushcn-x-corrected.pdf", width=12, height=6)

ushcn.cor <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="y", f=f, fprime=fprime, f2prime=f2prime, weight=1, ratio=1/60)
...
ushcn.cor2 <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="y", f=f, fprime=fprime, f2prime=f2prime, weight=.40, ratio=1/60)
