...
#   }
  ...
  f2p <- a*f2prime(x)
  v <- 1 + fp^2
  lambdap <- (sqrt(v^2-f2p*fp^2*ell) + v)^-1    
  lambdam <- -(sqrt(v^2+f2p*fp^2*ell) + v)^-1    
...
The quadratic correction breaks, whenever $(1 + f^\prime(x)^2)^2 \pm \ell\cdot f^{\prime\prime}(x)\cdot f^\prime(x)^2 < 0$
This is achieved by a combination of large values of $\ell$, which signify a large vertical extent, or large conditional variability $E[Y|X]$, and a large changes in the slop of the main trend, i.e large values of the curvature $f^{\prime\prime}(x)$. In the linear approximation of $f$ the same situation leads to a massive overcorrection of the vertical lines, making the `corrected' function almost unrecognizable. 
...
\[
\lambda_{\pm} = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}  = 2c\left(-b \pm \sqrt{b^2 - 4ac}\right)^{-1}
\]
...
\begin{eqnarray*}
|(\lambda)| &=& \ell \left(v + \sqrt{ v^2 \pm  f^{\prime\prime}(x_0) f^\prime(x_0)^2\cdot \ell}\right)^{-1} 
\end{eqnarray*}
for $v = 1 + f^\prime(x_0)$. 
