...
qplot(x=x, xend=x, y=y+ellx4.u, yend=y-ellx4.l, geom="segment", data=dframe, linetype=I(1)) +   
 theme_bw() + coord_fixed(ratio=1) + 
  ...
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), minor_breaks=minor.axis.correction,
                     labels=c("0", expression(paste(pi,"/2")), expression(pi), expression(paste("3",pi, "/2")), expression(paste("2",pi))))
@
\caption{Quadratic approximation based on eqns (\ref{eqn.q1}) and (\ref{eqn.q2}). \todo[inline]{we should either get rid of the scale on the y axis or nromailze the data afterwards. right now our quadratic approaximation is even changing the mean trend. }}
\end{subfigure}
...
This is achieved by a combination of large $\ell$ (i.e. large vertical extent) and a large value of curvature $f^{\prime\prime}(x)$, i.e large changes in the slope of the main trend. In the linear approximation of $f$ the same situation leads to a massive overcorrection of the vertical lines, making the `corrected' function almost unrecognizable. 
