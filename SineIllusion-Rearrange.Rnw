...
\newdo{Add an intro to this section: overview of what is addressed in this section: applet, study, results - one sentence each}
...
\newdo{The discussion on realistic situations has to go further back - this is not the place for it}
...
An applet demonstrating the effect of changing parameter values on the strength of the illusion as well as all of the correction factors we propose in this paper is made available at \url{http://glimmer.rstudio.com/srvanderplas/SineIllusion/}.
\newdo{Describe the functionality of the applet - half a  sentence for each item on the site. I know that people could just follow the link, but they usually won't.}
The applet used for data collection is scaled down to provide experimental control \newdo{what do you mean by experimental control?}; users are assigned specific transformations and starting values and can only increment or decrement the weight of the transformation.

Figure~\ref{fig:shiny.app} shows a screenshot of the applet used to collect user data. This applet is  available online at \url{http://glimmer.rstudio.com/srvanderplas/SineIllusionShiny/}. Both applets are implemented in {\tt shiny} \citep{shiny}. Users can modify the transformation weight using the plus and minus button (with plus indicating a higher level of correction).
...
\done{(should have been line length, not aspect ratio... sorry!)\\
...
\item Participants finished fewer than four trials: while participants were asked to complete twelve trials, some did not finish all of those. In order to stabilize predictions of random effects, participants' data was excluded, if there were fewer than four trials (XXX out of a total of \Sexpr{nparticipants} participants).
\item Out-of-bounds results: weights leading to severely over- or undercorrected results were excluded from the analysis. Figure~\ref{fig:overtransform} shows results at the threshold of acceptability. Only more severely distorted results were excluded from the analysis (XXX out of \Sexpr{ntrials} trials).
\end{itemize}
...
\end{align}
\newdo{Make the interpretation of $\alpha_X$ and $\alpha_Y$ as the lower threshold of plausible weights and  $\beta$ as the interval length of plausible weights  even more obvious, i.e. add a sentence to that effect :)}
...
temp<- xtable(fet[,c(5,6,1,4)])
align(temp) <- c("r", rep("c", 4))
print(temp, file="./figure/fixef.txt", floating=FALSE, include.rownames=FALSE)
