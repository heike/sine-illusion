...
#source("./code/ThemeBW.R") # redefine theme_bw() to have slightly darker minor gridlines
# this is not such a good idea - there's a reason that the minor gridlines are lighter
# right now the plots look like they are drawn on concstruction paper. 
# Please don't get stuck playing with these defaults. 
...
Ozone concentrations were gathered from \Sexpr{nsite} locations in the Houston area \citep{epa}, and weather data is aggregated from the NCDC \citep{noaa} site at Hobby International Airport, which is located in the center of the Houston area. 
...
\subsection{Application: Simulation}
\newdo{I'm not quite sure where this paragraph belongs, but it should probably not be a subsection of its own - it reads rather like a segue in or out of a section ...}
...
As the illusion is caused by a conflict between the vertical length and the width orthogonal to the slope, it can be resolved by making the orthogonal width constant. \newtext{This can be accomplished in several ways: by separating the trend and the variability, by forcing the slope to be constant (by reparameterizing the $x$ axis), or by making the lines appear to be of the same height by adjusting their lengths to maintain a constant orthogonal width. Each of these approaches is discussed in the following section.}
...
minor.axis <- seq(0, 2*pi, pi/8)
minor.axis.correction <- abs(cos(minor.axis))
minor.axis.correction <- minor.axis.correction/sum(minor.axis.correction)*2*pi
minor.axis.correction <- cumsum(minor.axis.correction)
...
minor.axis <- seq(0, 2*pi, pi/8)
minor.axis.correction <- rowMeans(cbind(abs(cos(minor.axis)), mean(abs(cos(minor.axis)))))
minor.axis.correction <- minor.axis.correction/sum(minor.axis.correction)*2*pi
minor.axis.correction <- cumsum(minor.axis.correction)
...
qplot(x=xtrans2, xend=xtrans2, y = ystart, yend=yend, colour=I("blue"), geom="segment", data=dframe) +
  ...
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), minor_breaks=minor.axis.correction,
                     labels=c("0", expression(paste(pi,"/2")), expression(pi), expression(paste("3",pi, "/2")), expression(paste("2",pi))))
@
\caption{Weighted $X$ axis transformation}
\label{xtrans2}
\end{subfigure}
...
minor.axis <- seq(0, 2*pi, pi/8)
minor.axis.correction <- rowMeans(cbind(abs(cos(minor.axis)), mean(abs(cos(minor.axis))), mean(abs(cos(minor.axis)))))
minor.axis.correction <- minor.axis.correction/sum(minor.axis.correction)*2*pi
minor.axis.correction <- cumsum(minor.axis.correction)

dframe$correction3 <- rowMeans(cbind(abs(cos(dframe$x)), mean(abs(cos(dframe$x))), mean(abs(cos(dframe$x)))))
dframe$correction3 <- dframe$correction3/sum(dframe$correction3)*2*pi
dframe$xtrans3 <- cumsum(dframe$correction3) # Create transformed X axis
...
qplot(x=xtrans3, xend=xtrans3, y = ystart, yend=yend, colour=I("blue"), geom="segment", data=dframe) +
  ...
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), minor_breaks=minor.axis.correction,
                     labels=c("0", expression(paste(pi,"/2")), expression(pi), expression(paste("3",pi, "/2")), expression(paste("2",pi))))
@
\caption{Weighted $X$ axis transformation\newline\hspace*{1.5em}(1 part transformation, 2 parts mean)}
\label{xtrans3}
\end{subfigure}
\begin{subfigure}[b]{.48\linewidth}\centering
<<xtrans-mid3,echo=FALSE, fig.width=4, fig.height=2>>=
minor.axis <- seq(0, 2*pi, pi/8)
minor.axis.correction <- rowMeans(cbind(abs(cos(minor.axis)), mean(abs(cos(minor.axis))), mean(abs(cos(minor.axis))), mean(abs(cos(minor.axis)))))
minor.axis.correction <- minor.axis.correction/sum(minor.axis.correction)*2*pi
minor.axis.correction <- cumsum(minor.axis.correction)

dframe$correction4 <- rowMeans(cbind(abs(cos(dframe$x)), mean(abs(cos(dframe$x))), mean(abs(cos(dframe$x))), mean(abs(cos(dframe$x)))))
dframe$correction4 <- dframe$correction4/sum(dframe$correction4)*2*pi
dframe$xtrans4 <- cumsum(dframe$correction4) # Create transformed X axis
...
qplot(x=xtrans4, xend=xtrans4, y = ystart, yend=yend, colour=I("blue"), geom="segment", data=dframe) +
  ...
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), minor_breaks=minor.axis.correction,
                     labels=c("0", expression(paste(pi,"/2")), expression(pi), expression(paste("3",pi, "/2")), expression(paste("2",pi))))
@
\caption{Weighted $X$ axis transformation\newline\hspace*{1.5em}(1 part transformation, 3 parts mean)}
\label{xtrans4}
\end{subfigure}
\todo[inline]{add 2nd example (maybe in the "application" section?), add dots to the axis?}
\caption{$X$ axis transformations. In (a), adjusting the x-spacing of the lines mitigates the sine illusion by changing the extant width so that it is uniform in $x$. (b) shrinks the transformation in (a) towards a uniform spacing and still breaks the illusion with a much less severe distortion of the $x$-axis.}
\label{xtrans}
\end{figure}
\clearpage
...
\end{figure}
\comment{The actual extant width is not as large as $m=\ell\cos\theta$ in this example. We're approximating the extant width with a simple geometric fix, but we could be more precise using law of cosines or by modifying the slope as in the linear and quadratic fixes.}
...
\todo[inline]{Something's strange here - minimum length should be 0.5, not 0.4}
\end{figure}
