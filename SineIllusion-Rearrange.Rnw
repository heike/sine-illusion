...
ggplot()  + geom_jitter(aes(Time, value/100), data=subset(ushcn, Year>1995), size=1) + 
  ...
  ylab("Temperature (in Celsius)") + theme_bw() + theme(plot.margin = unit(c(0,6,0,0), "cm"))
...
vp1 <- viewport(x = 0.95, y = 0.925, height = unit(2.5, "cm"), width =  unit(4, "cm"), just = c("right","top"))
...
% \begin{figure}[hbtp]
% \centering
% <<rawmap, dependson='readdata', echo=FALSE, echo=FALSE, fig.width=4, fig.height=2.5, out.width='.3\\textwidth',cache=TRUE>>=
% # ggsave(file="figure/monthly-temps.pdf", width=12, height=4.5)
% @
% % \includegraphics[width=\textwidth]{monthly-temps}
% \caption{Map of stations across the US included.}\label{fig:raw-map}
% \end{figure}
