...
The extent to which this weighting holds varies with the aspect ratio of the chart and the shape of the function. It also depends on the audience's experience with the sine illusion, resulting in very subjective choices of an
...
\label{fig:xtrans}
\end{figure}
\move{Note that we only make use of the transformation $T$ in the form of $f \circ T$. This means that we do not need to explicitly calculate the transformation $T$, which in particularly would involve computing the inverse of $f$ and might be computationally very intensive.}
\FloatBarrier
%\clearpage
...
Returning to the Ozone data in Figure \ref{fig:exampleFull1}, we will apply the x-correction to the loess fit line relating ozone concentration to daily temperature. Loess is particularly convenient for this transformation, as it enforces continuity conditions that include differentiability of the fitted function; software allows us to obtain the fitted $f^\prime(x)$ as well as $f(x)$. Figure \ref{fig:xtrans-example} shows the original data and the transformed $x$ axis.
