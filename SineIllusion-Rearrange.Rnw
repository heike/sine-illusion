...
\newdo{`can be found' - that sounds like you picked it up from the street - emphasize that this is part of the work you did for this paper. An applet ... is  made avalailable at ...}
...
The applet is implemented in {\tt shiny} \citep{shiny}. Users can modify the transformation weight using the plus and minus button (with plus indicating a higher level of correction).
...
ests <- data.frame(ests = c(fixef(model)[1:2], fixef(model)[1:2]+fixef(model)[3]), type=rep(c("X Transformation", "Y Transformation"), 2))
