...
qplot(x=xtrans, xend=xtrans, y = ystart, yend=yend, colour=I("blue"), geom="segment", data=dframe) +
  geom_segment(aes(x = 0, xend=2*pi, y=0, yend=0), colour="black") + theme_classic() + coord_fixed(ratio=1) + xlab(expression(tilde(x))) + ylab("y")+ 
...
\label{xtrans4}
\end{subfigure}
\todo[inline]{add 2nd example, add dots to the axis, add minor gridlines that are accurate.}
...
ggplot(data=df, aes(x=xstart, xend=xend, y=ystart, yend=yend)) + theme_bw() + coord_fixed(ratio=1) + 
  ...
  geom_segment(data=df[a,], aes(x=xend-.008, xend=x-.008, y=yend, yend=y), colour="grey40") + 
  geom_point(aes(x=x, y=y)) + 
  geom_segment(data=df[a,], aes(x=xstart, xend=xend, y=ystart, yend=yend)) + 
  geom_segment(data=dfsec, aes(x=sec.xstart+.005, xend=x+.005, y=sec.ystart-.005, yend=y-.005), colour="grey40") + 
  geom_segment(data=dfsec, aes(x=sec.xstart, xend=sec.xend, y=sec.ystart, yend=sec.yend)) + 
  geom_text(aes(x=dfsec$sec.xend, y=dfsec$sec.yend, 
...
\end{figure}
\comment{Note that the actual extant width is not as large as $m=\ell\cos\theta$ in this example. We're approximating the extant width with a simple geometric fix, but we could be more precise using law of cosines or by modifying the slope as in the linear and quadratic fixes.}
...
\caption{Correction factor in Eqn \ref{trigCorrection}}
\end{subfigure}
...
to obtain the lower endpoint $(x_l, y_l)$. With these two endpoints, we can correct separately for the top and bottom portions of the line, as the extant width is not symmetric when reflected across the original function f(x). \comment{Math language?}
...
#   df$seg.yend   <-  with(df, y-ell.orig/2*(1+cos(theta))+sec.ell2*cos(theta))
  ...
  df$seg.yend <- with(df, y-.5*(ell.orig)/sec.ell2)
  df
}
...
secantlines <- do.call("rbind", lapply(dframe$x, function(i) getSecantSegment(i, dframe, f, fprime, f2prime)))
...
#                               expression(paste("2",pi)))) + 
#   theme_bw() + theme(legend.position="bottom") + scale_linetype_discrete("Half-Segment")
# ggsave("./figure/fig-wtf.pdf", height=4, width=8)


# qplot(x=xstart, xend=xend, y = ystart, yend=yend, geom="segment", data=dframe2) +
#   theme_bw() + coord_fixed(ratio=1)+ 
#   theme(legend.position="bottom")  + xlab("") + ylab("") + facet_grid(.~type)+ 
#   scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), 
#                      labels=c("0", expression(paste(pi,"/2")), 
#                               expression(pi), 
#                               expression(paste("3",pi, "/2")), 
#                               expression(paste("2",pi))))
# ggsave("./figure/fig-CtsQuadratic.pdf", height=4, width=8)
@
\comment{Something very odd is going on with the CtsQuadratic chunk - I can't get either picture to compile correctly, but it runs fine on its own...? Figure \ref{HalfSegLength}}
\begin{figure}[h!tbp]\centering
\includegraphics[keepaspectratio=TRUE, width=.8\linewidth]{figure/fig-wtf}
\caption{Extant Half-segment Length. The secant  line segments are not symmetrical around the center line, so symmetric correction as employed in Figure \ref{GeneralLinearCorrection} leads to simultaneous over- and under-correcting, as in the distortions seen in Figure \ref{linearFixOvercorrect}}\label{HalfSegLength}
\end{figure}
