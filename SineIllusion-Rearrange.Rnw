...
\begin{itemize}
\item Participant did not interact with the applet: we required a participant to use the adjustment at least once in order to include data for this trial. (\Sexpr{})
