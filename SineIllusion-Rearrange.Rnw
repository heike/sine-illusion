...
\begin{subfigure}[b]{.45\linewidth}
  ...
  \label{fig:example2}
\end{subfigure}
\caption{\label{fig:exampleFull1} Ozone and Temperature in Houston, 2011. What seems to be the trend in overall variance? Is the variance trend in (a) consistent with the residual plot in (b)?}
\end{figure}
...
With an increase in the length of lines,  the apparent difference in the length decreases. Increasing the amplitude makes the difference in the lines appear more pronounced. The strength of the illusion is dependent on the ratio between the amplitude and the line length, in part because both of these parameters also influence the aspect ratio.
\newdo{how does the aspect ratio affect the illusion? this needs another sentence.}
\newdo{here's the first time the applet is mentioned - it gets mentioned again at the beginning of section 1.2 
Can we move this to just one place?}
...
\caption{The original Sine Illusion. The regions of the left graph in the peak and the valley appear to be composed of longer lines than the other regions.}\label{fig:original}
...
Another example of this phenomenon is shown in Figure \ref{fig:robbins-graph} \citep{robbins:2005}. The line-width illusion is also present in the graphs of $e^x$ and $e^x + 1$, as the lines appear to be separated by a much smaller distance at $x=2.5$ than at $x=0$.
...
Figure \ref{fig:original} shows a reproduction of Bach's applet \citep{bach} displaying the sine illusion. 
...
the sine illusion occurs due to misapplication of perceptual experience with the three-dimensional world to a two-dimensional ``artificial" display of data. One possible three-dimensional context for similar stimuli is shown in Figure \ref{fig:ribbon-illusion}. 
...
\caption{\label{fig:original-grid} Horizontal grid lines do not help  to resolve the illusion, even though they provide a clear basis for comparison of line lengths. Readers are able to correctly assess the lengths of the two singled out line segments as being equal. \newdo{the statement in the last sentence might be a bit strong.} }
\end{figure}
...
\newtext{What follows are several methods to ``break" the illusion's effect and allow for more accurate graphical conclusions.}
\done{ conclusion from the last paragraph is idea underlying this paper: we want to compensate the illusion. need to emphasize this a bit more.\\ -- Not sure what else to add - I could talk about the drawbacks but that would bias the reader.}
...
One approach to breaking the sine illusion is to present the trend and variance separately, as shown in Figure \ref{fig:trendremoval}. A loess smooth was used to estimate the trendline, and this estimate was subtracted from the line center to produce separate graphs of the trend and the variance. This is in line with  recommendations  for good graphics to `show the data' rather than make the reader derive some aspect of it \citep[e.g.][]{wainer:2000}. In particular, this is the generally accepted procedure for assessing model fit. 
...
\end{subfigure}
\caption{Separating the trend and the variability removes the Sine illusion. Figure \ref{fig:trendremovalFit} shows the fit line compared to the original function, Figure \ref{fig:trendremovalVariability} shows the the underlying fitted trendline and the leftover variability in the vertical lines which now appear to be of the same size.}\label{fig:trendremoval}
\end{figure}
...
$(0, \sum_{i=0}^K |f(x_i) - f(x_{i-1})|)$, to the range of $f$ on $(x_0, x_K)$.  This ensures that all pieces $f(T_i)$ have the same slope (of $|1|$).
\comment{I'm not actually sure this is true, mostly because empirically it seems like the slope is the average of the absolute slope (weighted by the length of the interval). }
\comment{It is. The average of the absolute slope comes in with the next sentence.}
We can then use another - global - linear scaling function to map from the range of $x$, i.e. interval $(x_0, x_K)$ to $(0, \sum_{i=0}^K |f(x_i) - f(x_{i-1})|)$, yielding a transformation function $T$ of
...
\end{equation}
This transformation is shown in Figure \ref{fig:xtrans1}.
...
By shrinking the transformation in Figure \ref{fig:xtrans1} towards the original $x$ values by a factor of $w \in (0,1)$, the illusion remains broken but the data is reflecting the  original function of $f(x)$ much more closely. We use the following equation for this: 
...
Figures \ref{fig:xtrans2} - \ref{fig:xtrans4} show the effect of different shrinkage coefficients $w$. The extent to which this weighting holds varies with the aspect ratio of the chart and the shape of the function. It also depends on the audience's experience with the sine illusion, resulting in very subjective choices of an
...
\label{fig:xtrans}
\end{figure}
\newdo{we don't need to explicitly calculate the inverse of f, we can do this implicitly by using the functional form of f. }
...
datasub$TmaxCorrect <- correctx(datasub$Tmax, fprime, a=45, b=95, shrink=0)
datasub$TmaxCorrect2 <- correctx(datasub$Tmax, fprime, a=45, b=95, shrink=1)


dots <- data.frame(Tmax=seq(45, 95, length=11), Ozone=0)
dots$TmaxCorrect <- correctx(dots$Tmax, fprime, a=45, b=95, shrink=0)
dots$TmaxCorrect2 <- correctx(dots$Tmax, fprime, a=45, b=95, shrink=1)

breaks <- data.frame(labels=as.character(seq(50, 90, 10)), breaks=seq(50, 90, 10))
breaks$breaks1 <- correctx(breaks$breaks, fprime, a=45, b=95, shrink=0)
breaks$breaks2 <- correctx(breaks$breaks, fprime, a=45, b=95, shrink=1)
...
Returning to the Ozone data in Figure \ref{fig:example}, we can apply the x-correction fairly easily as loess enforces continuity conditions that include differentiability of the smooth function. Figure \ref{fig:ozone-xcorrect} shows the original data and the transformed $x$ axis.
\comment{I think there's a way to get ggplot to transform only the axis, but I'm not sure that's what we want here... Opinions?}

\begin{figure}[h!]\centering
\begin{subfigure}[b]{.48\linewidth}\centering
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{xtrans-example1}
\caption{Original Data}\label{xtrans-example-original}
\end{subfigure}
\begin{subfigure}[b]{.48\linewidth}\centering
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{xtrans-example2}
\caption{Transformed X Axis}\label{xtrans-example-trans}
\end{subfigure}
\caption{The increasing variance is easier to see when $x$ has been transformed, because the function seems more linear.}
\end{figure}
...
\todo[inline]{Something's strange here - minimum length should be 0.5, not 0.6, and the aspect ratio wouldn't account for that discrepancy. I'm pretty sure if I can hunt this bug down, it might fix most of what's wrong.}
\end{figure}
...
\end{subfigure}
\caption{Changing aspect ratios changes the strength of the illusion and the effectiveness of the transformation}\label{fig:trigFix-aspectRatio} 
\end{figure}
...
Figures \ref{fig:raw-temp} and \ref{fig:resid-temp} show temperature data from 422 stations across the US with similar elevation and latitude. The first plot shows raw temperatures, the second residuals from a `baseline' model. The caption for the raw temperature plot suggests that the baseline model is not quite able to capture all of the variability in summer and winter peaks. However, the model used is actually based on averages in each time point. This model is not  susceptible to shrinking values, and it is our perception of the plot rather than the data that makes us see any higher variability in the peaks. After removing the averages in Figure \ref{fig:boxplot-temp} to yield Figure \ref{fig:boxplot-resid-temp} the impression of higher variability in the peaks is only partially confirmed: there is higher variability in the winter, and lower variability in the summer, though there are more outliers in the summer than in the winter.
