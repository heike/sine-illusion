...
\newdo{why do we look at starting weight in the first place? that needs a sentence}
\newtext{The study is set up as a fractional factorial design of correction type ($x$ or $y$ correction) and starting weight $w_0$. Each participant is asked to evaluate a total of twelve situations, six of each correction type. For the first six starting weights $w_0$ values were chosen  with equal probability from the set of numbers $\{0, 1, c\}$, where $c \sim U(.25, .75)$, while the second set of starting weights is randomly sampled from a distribution centered at 0.6 with mass on $(-4, 5)$. 
}
...
A single `trial' begins with the presentation of a graph at the chosen starting weight, and ends when the participant submits the trial. 
\newdo{What other records did you keep track of?}
