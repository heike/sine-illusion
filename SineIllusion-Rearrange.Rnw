...
\newdo{Add a paragraph on using a weighted approach to this correction.}
\newtext{Similar to the proposed $X$ transformation, we can use a weighted approach to find a balance between counteracting the illusion and representating the form of the original data. }
\newdo{state the formula with $w$ here}
\begin{equation}\label{eqn.ytrans.weighted}
\text{which function are you using in the app?}
\end{equation}
% % 
...
\newtext{The goal of the study is to determine an overall `optimal' setting for each of the correction factors. It is of additional interest, whether this setting is subject-specific or population-based, if the optimal setting depends on the starting value, and how much within-subject variability we find compared to between-subject variability. 
