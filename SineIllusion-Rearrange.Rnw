...
As with the $x$ correction, we can use a weighted approach to find a balance between counteracting the illusion and representating the form of the original data. In order to balance these goals for  the quadratic correction in $y$, half-segment lengths must be weighted separately; a single segment weighting will suffice for the linear correction in $y$. 
