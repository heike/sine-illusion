...
An applet demonstrating the effect of changing parameter values on the strength of the illusion as well as all of the correction factors we propose in this paper {\it can be found at} \url{http://glimmer.rstudio.com/srvanderplas/SineIllusion/}. This applet was used in collecting data from users and {\it can be found at} \url{http://glimmer.rstudio.com/srvanderplas/SineIllusionShiny/}. Figure~\ref{fig:shiny.app} shows a screen shot of it. 
\newdo{you cite two different websites. describe the difference, too.}
The applet is implemented in {\tt shiny} \citep{shiny}. Users can adjust the amount of adjustment using the plus and minus button (with plus indicating a higher level of correction).
\newdo{adjust, adjustment, adjust ... } Underlying this adjustment is the value of the weight $w$, as defined in eqns.~\ref{eqn.xtrans.weighted} and~\ref{eqn.ytrans.weighted}. 
\newdo{numerical feedback of weight is removed for the study, because results from a pilot study showed that users had `favorite' numbers for the weights and adjusted based on the number, not based on the plot. Not showing the number removed that bias (expectation error, meilgaard)}
...
The study is set up as a fractional factorial design of correction type ($x$ or $y$ correction) and starting weight $w_0$. Each participant is asked to evaluate a total of twelve situations, six of each correction type. For the first six starting weights $w_0$ values were chosen with equal probability from the set of numbers $\{0, 1, c\}$, where $c \sim U(.25, .75)$, while the second set of starting weights is randomly sampled from a distribution centered at 0.6 with mass on $(-4, 5)$, shown in figure \ref{fig:weightdist}. Participants were allowed to complete as many trials as they wished, with any trials after the first six sampled as in the second set of trials.
}
\newdo{a distribution centered at 0.6 - can you be more specific? is it a normal distribution? \\
Not exactly.}
\begin{figure}[h]\centering
\includegraphics[keepaspectratio=TRUE,width=.5\linewidth]{figure/fig-study-weight-dist}
\caption{Distribution of starting weights}\label{fig:weightdist}
\end{figure}
\newdo{center of 0.6 because of a results from a pilot study, which showed a  preference of 0.6 }
...
fixedmodel <- lm(endweight~(type-1)*training+startweight, data=lm.data)
...
ggplot() + theme_bw() + facet_grid(type~., scales="free") + 
  ...
  xlim(c(-.4, 1.4)) +
  geom_point(data=ints.all, aes(x=xmax, y=0, size=factor("Acceptable Weights")), shape=NA, colour="grey75") +
  guides(size=guide_legend("", override.aes=list(shape=15, size = 10), order=2)) +
  theme(legend.box="horizontal")

@

\newdo{From the writeup I'm not completely sure, which data you removed and why - simply removing outliers looks like you want to make your results prettier. You need a better reason for removing data. This is something that the reviewers will tear into, if you don't make a solid case.
...
\newtext{After removing data according to these criteria, we noticed that there were many responses for the $x$ correction that tended towards the endpoints of the weight value. These responses indicated that the participant did not attempt to maintain the underlying function, and the trials should thus be excluded from calculation of the optimal weight. This occurs as a result of an artifact of the x correction which enables participants to more easily perceive a three-dimensional image at corrections outside of [0,1] at the expense of the underlying mean function, as shown in figures \ref{fig:xtransover1} and \ref{fig:xtransover2}. }

<<xcorr-over, out.width='.45\\linewidth', fig.width=4, fig.height=2, dependson='xtransform', echo=FALSE, include=FALSE>>=
f <- sin
fprime <- cos
f2prime <- function(x) -sin(x)
dframe <- createSine(40 , len=1, f=f, fprime=fprime, f2prime=f2prime)
minor.axis.correction <- correctx(seq(0, 2*pi, pi/8), fprime, w=-3)

dframe$xtrans <- correctx(dframe$x, fprime=fprime, w=-3)
