...
\newdo{the \$\$ way of defining math environments is easy, but make a habit of actually numbering and naming important equations so that you can refer to it. }

\begin{equation}\label{eq.x.transformation}
T(x) = ...
\end{equation}

\newdo{the above definition of $T(x)$ still is not reflecting what you actually do in the code. Let's look at it from the other end:}

The idea of correcting for the illusion is to make all lines appear under the same slope, i.e. we want to find a transformation $\tilde{x}$ of $x$, such that $f(\tilde{x})$ is a  piece-wise linear function, where each piece has the same absolute slope. Since the slope is determined by the aspect ratio, we are free to choose it and w.l.o.g. we get for each piece $T_i$: 
\[
f(T_i(x)) = \pm a x + b_i.
\]
...
Let function $s = s_{[a,b]}^{[c,d]}$ be the linear scaling function that maps  the interval $(a,b)$ linearly to the interval $(c,d)$. This function is defined as
\[
s(x) = s_{[a,b]}^{[c,d]} (x) = (x-a)/(b-a) \cdot (d-c) + c.
\]
...
\[
s_{[e,f]}^{[c,d]} \otimes s_{[a,b]}^{[e,f]} = s_{[a,b]}^{[c,d]}
\]


In our situation let the scaling Function $s$ be given as :
\[
s_{[c,d]}^{\range{f(x)}}(x) = f(x_{i-1}) + (x-c)/(d-c) \cdot (f(x_{i}) - f(x_{i-1}))).
\]
...
T_i(x) &=& f^{-1}\left( s_{[c_i,d_i]}^{\range({f(x)}})(x) \right). 
\end{eqnarray}

\newdo{still need to show that $f(T(x)$ is a linear function, i.e. compute $a$ (in particular this $a$ has to be the same for all $i$ -- here it will turn out to be 1) and $b_i$)}

Note that parameters $c$ and $d$ determine the speed with which the transfomation happens, i.e. for $x \in (c,d)$ we have
\[
f(T(x)) = f (f^{-1}(s_{[c_i,d_i]}^{\range({f(x)}}) (x))) = f(x_{i-1}) + (x-c_i)/(d_i-c_i) \cdot (f(x_{i}) - f(x_{i-1}))).
\]
Correspondingly, the slope of $f(T_i(x))$ is given as $(f(x_{i}) - f(x_{i-1})))/(d_i-c_i)$.
...
\begin{itemize}
\item for $[c_i, d_i] = \range(f(x))$ the new $x$-range is the original $y$ range, but the scaling function simplifies to the identity.

\item in order to preserve the original $x$-range, we can choose $[c_i, d_i]$ as .... . 
\end{itemize}

\newdo{this is a good motivation, but put it before the math.}
\newtext{
This transformation can be considered as a reparameterization of the $x$ axis in terms of the slope. As the sine illusion depends on changing slope in the overall trend, by reparameterizing the $x$-axis in terms of the slope, the data seem to have constant slope, removing the illusion, while the $x$-axis shows the changing slope. The transformation is shown in Figure \ref{xtrans1}. An analysis of $T(x)$ is shown in appendix \ref{XtransAppendix}.}
...
\label{xtrans}
\end{figure}
\todo[inline]{add 2nd example (maybe in the "application" section?)}
\clearpage
...
qplot(data=ribbon, x=x, ymin=ybottom, ymax=ytop, geom="ribbon", fill=I("grey")) + 
  ...
  theme(axis.title=element_blank(), axis.text=element_blank(), axis.ticks=element_blank(), 
        axis.line=element_blank(), panel.grid=element_blank(), legend.position="right", legend.direction="vertical") + 
  ...
  coord_equal(ratio=1) + scale_linetype_discrete("")
@
\begin{figure}[h!tbp]
\begin{center}
...
\end{center}
\caption{\label{fig:linewidth}Sketch of line width assessments: The solid line shows vertical width, the dotted line shows  width orthogonal to the slope. \comment{From previous surveys (we could cite Marie's survey here) we know that  observers associate line width more with  orthogonal width than vertical width.}}
\end{figure}
% 
...
\newtext{If this is the case, then it may be  possible to correct the graphical display for so that it is not as visually deceptive. Our first approach to this is to compare the (perceived) orthogonal width to the (information-carrying) vertical length of the line. By adjusting the vertical line such that the orthogonal width displays the quantity of interest, we can match the audience's intuitive expectation with the data. This is still dependent on having a functional form for $f(x)$ which has derivative $f^\prime(x)$ on the range of $x$ within the data, as we will determine $\theta$ based on the slope of the secant line, $-1/f^\prime(x)$.} 
\subsubsection{Trigonometry Method}\label{trig}
This approach aims to extend the orthogonal width, which onlookers generally evaluate,  to the amount of the vertical width, which we want to represent. While previous work in \citet{schonlau:2003} used this adjustment for a line with constant slope, we must extend this approach to $f(x)$ with non-constant derivative $f^\prime(x)$ which may vary continuously. 
...
qplot(data=df.real, x=x, y=y, geom="line") + theme_minimal() + coord_equal(ratio=1) +
  ...
  geom_text(aes(x=mean(c(dfsec$sec.xstart, df$xend[a], df$x[a], df$x[a]))+.04, 
                ...
                label="theta"), parse=TRUE, size=ftsize+1) +
  geom_text(aes(x=df.real$x[which.min(df.real$sec)], y=min(df.real$sec, na.rm=TRUE)-.05, 
                label="slope: paste(-1/f, \"'\", (x))"), parse=TRUE, hjust=.75, vjust=1) +
  geom_text(data=df[a,], aes(x=.5*xend-.05 + .5*x, y=.5*yend+.5*y, label="paste(frac(\u2113, 2))"), size=ftsize+1, face="bold", parse=TRUE) +
  geom_text(aes(x=.5*dfsec$sec.xstart+ .5*dfsec$x +.05 , y=.5*dfsec$sec.ystart+.5*dfsec$y-0.03, label="italic(m)"), parse=TRUE, face="plain") + 
  xlab("x") + ylab("y") +
...
\caption{Geometry of the trigonometry correction approach. $\ell$ denotes the length of the entire vertical segment, hence, $\ell/2$ is the length of the portion above the mean function.}
\end{figure}
\todo[inline]{
...
we can solve for $\theta$ to get $\atan{\left|f^\prime(x)\right|} = \theta$. 
Then 
$\cos(\theta) = \cos(\atan{\left|f^\prime(x)\right|}) = m/\ell$ and 
$$\left[\cos(\atan{\left|f^\prime(x)\right|})\right]^{-1} = \ell/m$$ That is, we have to scale $\ell$ by a factor of 
$\left[\cos(\atan{\left|f^\prime(x)\right|})\right]^{-1}$ to make the extant width $m$ appear as long as $\ell$ in the original graph. 
...
\caption{Trigonometic correction}
\end{subfigure}
...
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{figure/fig-generalcorrectioncartoon1}
\caption{General Correction}
\label{GeneralCorrection}
\end{subfigure}
...
\end{subfigure}
\end{minipage}
\caption{General correction approach. (a) may require numerical optimization to obtain exact solutions for $(x_u, y_u)$ and $(x_l, y_l)$. (b) uses a first-order taylor series approximation to $f(x)$ and (c) uses a second-order taylor series approximation to $f(x)$. The intersection of the function $f(x) \pm \ell/2$ and the secant line,  $(x_u, y_u), (x_l, y_l)$ must be obtained to determine the necessary correction factor.}\label{GeneralCorrection-all}
\end{figure}

As shown in Figure \ref{GeneralCorrection-all}, another method for adjusting the vertical length to ensure constant extant width is to solve the systems of equations\\
...
dfslope <- data.frame(xstart=df$x[a]+.5, xend=df$x[a]-.9, ystart=fprime(df$x[a])*.5+df$y[a], yend=-.9*fprime(df$x[a])+df$y[a])
dfslope2 <- data.frame(xstart=df$x[a]+.5, xend=df$x[a]-.3, ystart=fprime(df$x[a])*.5+df$y[a]+.5, yend=-.3*fprime(df$x[a])+df$y[a]+.5)
qplot(data=df.real, x=x, y=y, geom="line") + theme_minimal() +
  geom_line(data=df.real,aes(y=yupper), linetype=3) + 
  geom_line(data=df.real,aes(y=ylower), linetype=4) + 
  geom_segment(data=df, aes(x=xstart, y=ystart, xend=xend, yend=yend), colour="grey50") + 
  geom_segment(data=df[a,], aes(x=xstart, y=ystart, xend=xend, yend=yend), colour="black", size=1.5)+
...
1/\left(\sqrt{1 + f^\prime(x)^2}\right) = (\atan(f^\prime(x)))^\prime/f^{\prime\prime}(x)
\end{equation}
...
As shown in Figure \ref{quadratic-GeneralCorrection}, solving the system of equations provides endpoints $(x_l, y_l)$ and $(x_u, y_u)$ which can then be used to compute the extant width of each half-segment $m$. Unlike the linear solution, the half-segments here are not of the same length, as shown in Figure \ref{HalfSegLength}, and thus there are separate correction factors for each half-segment. The correction factors are $(\ell/2)/(m_u)$ and $(\ell/2)/(m_l)$ respectively. 
...
qplot(x=x, xend=x, y=y+ellx3.u, yend=y-ellx3.l, geom="segment", data=dframe, linetype=I(1)) + coord_fixed(ratio=1) + theme_bw() + ylab("y") + 
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), 
                     labels=c("0", expression(paste(pi,"/2")), expression(pi), expression(paste("3",pi, "/2")), expression(paste("2",pi))))
@
