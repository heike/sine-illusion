...
\caption[General correction approach]{\comment{This looks wrong - could you check the code, again?} General correction approach. This approach may require numerical optimization to obtain exact solutions for $(x_u, y_u)$ and $(x_l, y_l)$.}\label{fig:GeneralCorrection}
\end{SCfigure}
