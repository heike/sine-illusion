...
createSine <- function(n=200, len=1, f=f, fprime=fprime, f2prime=f2prime, a=0, b=2*pi) {
  ...
  v = 1 + fprime(x)^2
  i2 <- 0.5*(v+sqrt(v^2 + ell*f2p*fp^2))/sqrt(v) # small side - on the outside
  o2 <- abs(0.5*(-v+sqrt(v^2 - ell*f2p*fp^2)/sqrt(v))) # large side on the inside
  dframe$ellx4.l <- o2
  dframe$ellx4.u <- i2
  idx <- which(f2p < 0)
  dframe$ellx4.l[idx] <- i2[idx]
  dframe$ellx4.u[idx] <- o2[idx]
    
  k <- which(diff(f2p < 0) != 0)
  yoff <- max(i2+o2)/2  # not sure whether this works in practice - if 
...
qplot(x=x, xend=x, y=y+ellx4.u, yend=y-ellx4.l, geom="segment", data=dframe, linetype=I(1)) +   
 theme_bw() + coord_fixed(ratio=1) + 
  ...
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), minor_breaks=minor.axis.correction,
                     labels=c("0", expression(paste(pi,"/2")), expression(pi), expression(paste("3",pi, "/2")), expression(paste("2",pi))))
@
\caption{Quadratic approximation based on eqns (\ref{eqn.q1}) and (\ref{eqn.q2}). \todo[inline]{we should either get rid of the scale on the y axis or normalize the data afterwards. right now our quadratic approaximation is even changing the mean trend. }}
\end{subfigure}
