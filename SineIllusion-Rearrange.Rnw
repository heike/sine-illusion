...
Let $W_{ij}$ denote the final adjustment to weight by participant $i$, $ 1 \le i \le \Sexpr{nparticipants}$ \newdo{We can't use nparticipants - that's too many, we need to get rid of the invalid ones ... I think we only have 126 actual participants}, on trial $j$, $1 \le j \le n_i$. 
...
ushcn.cor <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="x", weight=1)
...
ushcn.cor2 <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="x", weight=.36)
...
ushcn.cor <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="y", weight=1)
...
ushcn.cor2 <- correct("time", "temp", subset(ushcn[,-c(4, 5)], Year>2008), type="y", weight=.40)
