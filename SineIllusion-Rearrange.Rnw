...
\done{add a 2nd example here - how about a correction for the ozone data from the beginning?}
%\clearpage
...
Returning to the Ozone data in Figure \ref{fig:exampleFull1}, we can apply the x-correction fairly easily as loess enforces continuity conditions that include differentiability of the smooth function. Figure \ref{fig:xtrans-example} shows the original data and the transformed $x$ axis.
...
\end{subfigure}
\caption{The increasing variance is easier to see when $x$ has been transformed, because the function seems more linear.}\label{fig:xtrans-example}
\end{figure}
Figure \ref{fig:xtrans-example} demonstrates not only the effect of transformation of the $x$-axis, but also that the transformation is not particularly misleading, as the granularity of the data in this example provide an implicit measure of the strength of the transformation along the x-axis and the transformation is also clearly evident in the labels along the $x$-axis. While we typically prefer that our axis transformations are monotone, this transformation provides additional utility that a monotone transformation does not.
