...
The function describing the orthogonal line through $(x_o, f(x_o))$ is given in intrinsic form as 
\[
{x_o \choose f(x_o)} + \lambda {f^\prime(x_o) \choose 1}, 
\]
for any real-valued $\lambda$.
By setting the above equation equal to the graphs of $f_u$ and $f_\ell$ we find end points of the line segment as solutions in $x$ and $\lambda$ of the system of equations:
\begin{eqnarray}\label{eqn.general}
(I) && x - x_o = \lambda f^\prime(x_o)\\ \label{eqn.general.2}
(II) && f(x) - f(x_o) = -\lambda \pm \ell/2
\end{eqnarray}
With these two endpoints, we can  separately correct for the top and bottom portions of the vertical width. Note that the extant width generally is   not symmetric when reflected across the original function $f(x)$ (as can be seen in figure \ref{GeneralCorrection}). 
...
Taylor approximations can be employed to reduce the computational requirements. Approximations derived from the first and second-order taylor approximations are described in the following sections.
...
Using a linear approximation of $f(x)$ in value $x_0$, i.e.  
 ...
 \begin{eqnarray*}
 (II)  &&  f^\prime(x_0)(x-x_0) = -\lambda \pm \ell/2,
 \end{eqnarray*}
...
\newdo{why are our solutions in the code not the same? they are only linearly dependent values. }
...
Using the approximation $f(x) \approx f(x_0) + f^\prime(x_0)(x-x_0) + 1/2 f^{\prime\prime}(x_0)(x-x_0)^2$, the system of equations  \ref{en.general} and \ref{en.general.2}  simplifies to the following  quadratic equation in $x$:
\begin{eqnarray*}
(x-x_0)  f^\prime(x_0) + 1/2 (x-x_0)^2 f^{\prime\prime}(x_0) = - (x-x_0) \left(f^\prime(x_0)\right)^{-1} \pm \ell/2.
\end{eqnarray*}
