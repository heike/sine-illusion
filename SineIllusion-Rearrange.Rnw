...
# cars <- read.csv("http://www.epa.gov/otaq/cert/mpg/testcars/database/13tstcar-2012-11-09.csv")
# qplot(ETW, FE.Bag.3, data=cars) + geom_jitter()

# Dataset with outliers present
airquality2 <- airquality[!is.na(airquality$Ozone),]
airquality2$fit <- loess(data=airquality2, Ozone~Temp)$fitted
airquality2$resid <- loess(data=airquality2, Ozone~Temp)$resid
# Group data by quantiles, compute residual variance for each quantile to see if the error variance is similar
airquality2$group <- sapply(airquality2$Temp, function(i) sum(i<=(quantile(airquality2$Temp, c(.25, .5, .75, 1)))))
airquality2 <- ddply(airquality2, .(group), transform, groupvar=var(resid))
airquality2 <- airquality2[order(airquality2$Temp),]

# Dataset with outliers removed
airquality3 <- airquality2[which(abs(airquality2$resid)<50), ]
airquality3$fit <- loess(data=airquality3, Ozone~Temp)$fitted
airquality3$resid <- loess(data=airquality3, Ozone~Temp)$resid
# Group data by quantiles (using quantiles from unaltered dataset), 
# compute residual variance for each quantile to see if the error variance is similar
airquality3$group <- sapply(airquality3$Temp, function(i) sum(i<=(quantile(airquality2$Temp, c(.25, .5, .75, 1)))))
airquality3 <- ddply(airquality3, .(group), transform, groupvar=var(resid))
airquality3 <- airquality3[order(airquality3$Temp),]

qplot(data=airquality2, x=Temp, y=Ozone) + geom_line(aes(y=fit)) + theme_bw()
qplot(data=airquality3, x=Temp, y=Ozone) + geom_line(aes(y=fit))+ theme_bw()

qplot(data=airquality2, x=Temp, y=Ozone) + geom_line(aes(y=fit)) + geom_ribbon(aes(ymax=fit+2*sqrt(groupvar), ymin=fit-2*sqrt(groupvar)), alpha=.25, colour="grey")+ theme_bw()
qplot(data=airquality3, x=Temp, y=Ozone) + geom_line(aes(y=fit)) + geom_ribbon(aes(ymax=fit+2*sqrt(groupvar), ymin=fit-2*sqrt(groupvar)), alpha=.25, colour="grey")+ theme_bw()
qplot(data=airquality2, x=Temp, y=resid) + geom_line(aes(y=0*fit)) + geom_ribbon(aes(ymax=2*sqrt(groupvar), ymin=-2*sqrt(groupvar)), alpha=.25, colour="grey") + ylab("Residual")+ theme_bw()
qplot(data=airquality3, x=Temp, y=resid) + geom_line(aes(y=0*fit)) + geom_ribbon(aes(ymax=2*sqrt(groupvar), ymin=-2*sqrt(groupvar)), alpha=.25, colour="grey") + ylab("Residual")+ theme_bw()
@

\done{We need a relevant example as a hook - then explain to people what trap they fell in.}

Figure \ref{example1} shows daily measurements of Ozone concentration and temperature in New York from May to September, 1973 with a loess smooth line for reference. What seems to be the trend in overall variance? Figure \ref{example2} shows the same data with the three most heinous outliers removed. What seems to be the trend in overall variance in Figure \ref{example2} compared to the trend in overall variance in Figure \ref{example1}?

\begin{figure}[h!tbp]\centering
\caption{Ozone and Temperature in New York, May-September 1973}
...
Figure \ref{exampleFull2} shows the same data, but includes a shaded region that is 2$\sigma_q$ above and below the fitted line, computed separately for each quartile $q=1, 2, 3, 4$ of temperature. It is much more obvious now that the variance is actually increasing with temperature, and in fact, the variance in the 4th quartile is \Sexpr{round(mean(airquality3$groupvar[which(airquality3$group==1)])/mean(airquality3$groupvar[which(airquality3$group==4)]), 2)} times that of the 1st quartile, even when outliers are removed. Without the outliers, the increasing variance might be more obvious, but is still camoflauged by the increasing trend.  
\begin{figure}[h!tbp]\centering
\begin{subfigure}[b]{.45\linewidth}
  \centering
  \includegraphics[width=\textwidth]{figure/fig-example3}
  \caption{\small All data}
  \label{example3}
\end{subfigure}
\begin{subfigure}[b]{.45\linewidth}
  \centering
  \includegraphics[width=\textwidth]{figure/fig-example4}
  \caption{\small Outliers removed}
  \label{example4}
\end{subfigure}
\caption{Variance in Ozone levels with Temperature, New York (May-September 1973). Shaded bands are 2$\sigma$ above and below the fit line, computed separately for each temperature quartile. Even with the outliers removed, the variance for  days above 85 degrees is much higher than the variance for days below 70 degrees.}
\label{exampleFull2}
\end{figure}
\comment{I left the bands around the trendline because it's more obvious there than without the bands that there is increasing variance, but obviously the bands are still effected by the illusion. Not sure what's best there...}
\begin{figure}[h!tbp]\centering
\begin{subfigure}[b]{.45\linewidth}
  \centering
  \includegraphics[width=\textwidth]{figure/fig-example5}
  \caption{\small Residuals}
  \label{example5}
\end{subfigure}
\begin{subfigure}[b]{.45\linewidth}
  \centering
  \includegraphics[width=\textwidth]{figure/fig-example6}
  \caption{\small Residuals with outliers removed}
  \label{example6}
\end{subfigure}
\caption{Residual variance in Ozone levels with Temperature, New York (May-September 1973), with bands showing the increase in variance as temperature increases}
\label{exampleFull3}
\end{figure}

Even with the variance bands, it is difficult to truly comprehend the magnitude of the change in variance until the residuals are plotted without the underlying trend. This is shown in Figure \ref{exampleFull3}. The line width illusion is a frequent occurrence in statistical analysis, and graphical displays should be thoughtfully considered to minimize its effect visually and acknowledge its influence even when the effect cannot be entirely mitigated through graphical means.

\comment{I've restructured the introduction a bit to better transition from "line width illusion" to sine illusion (at least, I think it flows a bit better)}

\subsection{Statistical Graphics and the Line Width Illusion}\label{statisticalgraphics}
...
% \end{figure}
\clearpage
\newtext{The line width illusion can become even more misleading when it occurs along a periodic function, where the variance may appear to follow a periodic trend when it is in fact constant. This more specific instance of the line width illusion, the sine illusion, is shown in Figure \ref{original}.}
...
Sections \ref{perceptualexplanations} and \ref{statisticalgraphics} explain the related line width illusion and sine illusion. While these illusions are not entirely separate, and similar visual processing heuristics likely underly both illusions, the size of the effect can be vastly increased when the two are superimposed. This occurs frequently in time series with both trend and seasonality, but for the sake of simplicity, we will use simulated data to demonstrate the effect. 
\begin{figure}[h!tbp]\centering
...
@
  ...
  \label{simulation3}
\end{subfigure}
\caption{The sine illusion and the line width illusion describe overlapping phenomena which may manifest on different orders of magnitude within the data but can both be visually misleading. Figure \ref{simulation1} shows the sine illusion without any underlying trend, Figure \ref{simulation2} shows the sine illusion superimposed on a quadratic trend, and Figure \ref{simulation3} shows a quadratic trend without the sine illusion. Though all three sets of simulated data have constant variance, Figure \ref{simulation2} seems more extreme.}
\end{figure}
...
#             data=dframe.1, parse=TRUE, hjust=1, vjust=.5, size=2.5) + 
  geom_text(aes(label=paste("theta", "%~~%", round(vangle/pi*180), "^o", sep=""),
            ...
            data=dframe.1, parse=TRUE, hjust=.75, vjust=.5, size=2.5) + 
...
\end{figure}
The sine illusion disappears when the width orthogonal to the slope is made constant. \newtext{This can be accomplished by separating the trend and the variability, by forcing the slope to be constant (by reparameterizing the $x$ axis), or by making the lines appear to be of the same height, by adjusting their lengths to maintain a constant orthogonal width. Each of these approaches is discussed in the following section.}
...
\begin{figure}[h!tbp]\centering
<<trend-removal, include=FALSE, fig.height=3, fig.width=3.5>>=
...
% \end{subfigure}
\caption{Fix of the sine illusion by using the factor of $1/\cos\left(\atan{|f^\prime(x)|}\right)$} to adjust the vertical extent of the lines.  
\label{trigFix} 
\end{figure}
...
@
\comment{Something very odd is going on with the CtsQuadratic chunk - I can't get either picture to compile correctly, but it runs fine on its own...? Figures \ref{altOrthogonalWidth} and \ref{HalfSegLength}}
\begin{figure}[h!tbp]\centering
