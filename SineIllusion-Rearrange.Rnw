...
\end{figure}
%
An applet demonstrating the effect of changing parameter values on the strength of the illusion as well as all of the correction factors we propose in this paper is available at \url{http://glimmer.rstudio.com/srvanderplas/SineIllusion/}. Users can examine the sine illusion, changing line length and function amplitude, and compare corrections in $x$ and $y$ to uncorrected data. 

In addition, figure~\ref{fig:shiny.app} shows a screenshot of the applet used to collect user data. This applet is also available online at \url{http://glimmer.rstudio.com/srvanderplas/SineIllusionShiny/}. This applet presents users with a graph that is the result of a correction in $x$ or $y$ with a randomly selected starting weight value. Underlying this adjustment is the value of the weight $w$ as defined in eqns.~\ref{eqn.xtrans.weighted} and~\ref{eqn.ytrans.weighted}, though the numerical value of $w$ was hidden from the user to prevent anchoring to a specific numerical value.  The applet used in data collection is much more strictly controlled (as users cannot select the correction type or starting weight value) to allow for random assignment of trial order.  

As the line length and function are controlled in this app, we used the linear transformation in $y$; the transformation does not break under any combination of parameters tested in this experiment. 
...
ints$label1 <- c("alpha", "alpha + beta", "alpha", "alpha + beta")
ints$label <- sprintf("%s <= ~ %s <= %s", round(ints$xmin, 2), ints$label1, round(ints$xmax, 2))
...
\newdo{not sure if this still needs to be in here}
...
\begin{figure}[htbp]\centering
<<meanDists,echo=FALSE,out.width='.8\\linewidth', dependson='results-analysis', fig.width=5, fig.height=3, message=FALSE, warning=FALSE>>=
...
@
\caption{Distribution of participant-level means using the standard method of limits analysis described in \citet{goldstein}. The overall means are both near 0.4, however, there is quite a bit of user-level variability. }
\end{figure}
As a result, according to this analysis, the optimum transformation value for $x$ is \Sexpr{round(user.avg.x, 2)}, and the optimum transformation value for $y$ is \Sexpr{round(user.avg.y, 2)}. In addition, only \Sexpr{nrow(subset(user.avg.all, avg<0 | avg > 1))} user averages were outside of the range of (0,1) for either trial. While these results suggest that the transformation is useful and that complete transformation is not necessary, we can get more precise bounds on the range of acceptable transformation weights using a linear model that can incorporate starting points other than 0 and 1, as well as data from users that completed a sufficient number of trials but did not complete a trial starting at one of the endpoints. 
...
$\mathbbm{1}$ is the indicator function, that is, when the condition is true, the function is equal to 1, otherwise, the function is equal to 0. Below, $\alpha_X$ is used to model $W_{ij}$ if $T_{ij} = \text{"X"}$, otherwise, $\alpha_Y$ is used.
