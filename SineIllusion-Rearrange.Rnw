...
asp <- ((4/3)/70)/(10/12) # image vs coordinate aspect ratio 
startyear <- 2000
...
ggplot(aes(x=value, y=temp, group=variable), data=ushcn.cor.x) + 
  geom_jitter(size=1, position = position_jitter(width = 1/36), alpha=.75)+ 
  facet_grid(variable~.) +
  geom_line(data=ushcn.cor.x, aes(y=temp.fit), colour="steelblue", size=1.5) + 
  xlab("Year") +  
