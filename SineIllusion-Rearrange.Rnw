...
\item forcing the slope to be constant (by reparameterizing the $x$ axis), or 
\item making the lines appear to be of the same height by adjusting their lengths to maintain a constant orthogonal width. 
\end{enumerate}
Each of these ideas is discussed in the following section.
