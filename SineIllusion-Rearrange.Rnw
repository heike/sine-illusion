...
Another approach to breaking the power of the sine illusion is to remove the changes in the slope of the overall trend line by transforming the $x$ axis scaling so that the absolute slope is constant. 
Geometrically, we want to change the x-axis spacing so that the absolute slope of the underlying trend line remains constant, which will force the extant width to be constant. \newtext{This transformation assumes that we have an underlying function $f$ (either due to prior knowledge or using a model fit) which is differentiable over the region of observed data. If data $(x_i, y_i)_{i=1, ..., N}$ have been observed, then we assume we can model $y_i = f(x_i) + \epsilon_i$ and that $f^\prime(x_i)$ exists for all $i=1, ..., N$. }
...
Let function $s$ be the linear scaling function that maps  the interval $(c, d)$ linearly to the interval  defined by the end points $f(x_{i-1})$ and $f(x_i))$. Function $s$ is given as :
\[
s_{[c,d]}(x) = f(x_{i-1}) + (x-c)/(d-c) \cdot (f(x_{i}) - f(x_{i-1}))).
\]
...
T_i(x) &=& f^{-1}\left( s_{[c_i,d_i]}(x) \right). 
\end{eqnarray}
...
\[
f(T(x)) = f (f^{-1}(s_{[c_i,d_i]} (x))) = f(x_{i-1}) + (x-c_i)/(d_i-c_i) \cdot (f(x_{i}) - f(x_{i-1}))).
\]
...
\begin{itemize}
\item for $c_i = \min{(f(x_{i-1}), f(x_i))}$ and $d_i = \max{(f(x_{i-1}), f(x_i))}$ the new $x$-range is the original $y$ range. 

\item an additional scaling reflecting the ratio of original $y$- and $x$-ranges will result in the  $x$-range being preserved. This happens  for $c_i = \range(x)/\range(f(x)) \cdot \min{(f(x_{i-1}), f(x_i))}$ and $d_i = \range(x)/\range(f(x)) \cdot \max{(f(x_{i-1}), f(x_i))}$. 
\end{itemize}
