...
\newtext{A comparison of the sketches for the linear approach (in Fig. \ref{fig:linear-GeneralCorrection}) and the trigonometric approach (in Fig. \ref{fig:trig}) shows that they are the same. They do, indeed, lead to the same correction factor.}
% 
...
\end{equation}
This results in a correction factor for the half lengths as:
\begin{equation}\label{eqn.quadratic}
\ell_{\text{new}} = \left( 1 + \sqrt{1 + (\pm \ell_{\text{old}}) \frac{f^\prime(x_0)^2 f^{\prime\prime}(x_0)}{\left(1 + f^\prime(x_0)^2\right)^2})} \right) \left(1 + f^\prime(x_0)^2\right)^{-1/2}
\end{equation}
% \begin{eqnarray*}
