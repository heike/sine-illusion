...
@
\newdo{Could you make this a table of means with 95\% CI?}
...
\newdo{define ranges of i and j - how many participants did we have after the cleaning? - we should state that above.}
\newtext{Let $W_{ij}$ denote the final adjustment to weight by participant $i$, $ 1 \le i \le XXX$, on trial $j$, $1 \le j \le n_i$. }
