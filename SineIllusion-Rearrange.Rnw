...
More recently, the line-width illusion has been cited as a reason to modify hammock plots...\todo[inline]{add more here...} Figure \ref{hammocks} shows the hammock plot and the corrected plot, which has been modified so that all angles are constant.
\begin{figure}[htbp]
\centering
\missingfigure{Figure or figures showing hammock plots and the corrected version}
\caption{}
\label{hammocks}
\end{figure}
...
\clearpage
\section{Breaking the Sine Illusion by Mathematical Transformations}
\subsection{Hammock-Plot Transformations}
\subsection{Transformation of the X-Axis}
\subsection{Transformation of the Y-Axis}
\subsubsection{Linear Approximation to f(x)}
