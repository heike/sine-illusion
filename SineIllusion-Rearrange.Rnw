...
%\clearpage
\section{Conclusion}
The sine illusion is a persistent and powerful illusion that is very difficult to counteract without modifying the visual stimulus directly. While ``fixing" data is anathema to typical statistical procedures, we cannot counteract the illusion and represent the data visually without some intervention. The proposals in this paper for transformations in $X$ and $Y$ provide the means to temporarily ``fix" the data as a diagnostic measure, perhaps using an applet or R package for that purpose. These corrections are significant not only because of their implications for statistical graphics, but because previously attempts to resolve optical illusions using geometry have not met with success \citep{westheimer2008illusions}. In addition, while the corrections are imperfect and break down at extreme secant values, we are only performing the first step of what should be an iterative procedure; multiple iterations will likely resolve some of the more difficult functions.

Our primary goal is to raise awareness of the illusion and its implications for statistics; the use of plots to guide the modeling process can leave us vulnerable to overlooking changes in the variance due to the illusion. While best practice has  been to plot the residuals separately, this removes the context of the data and is not practical before there is a model. The proposed transformations require only a nonparametric smooth, maintain the context of the data, and are not out of place in the visual arts; the columns of the Parthenon are wider at the center than at the ends to ensure that they appear straight, similarly, painters often correct the hues used in shadows to account for color constancy illusions. In addition, the cognitive load required to interpret residuals separately from a model line can be reduced by showing the corrected data and model line together on the same plot. 
