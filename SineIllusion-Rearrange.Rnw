...
Figure \ref{example1} shows daily measurements of 8-hour average ozone concentration and temperature at several sites in Houston, for days in 2011 with temperatures above $45^\circ$F  and dew points of less than $60^\circ$F. %(as ozone concentration is related to temperature and humidity)
...
\begin{subfigure}[b]{.45\linewidth}
  ...
  \label{example2}
\end{subfigure}
\caption{\label{exampleFull1} Ozone and Temperature in Houston, 2011. What seems to be the trend in overall variance? Is the variance trend in (a) consistent with the residual plot in (b)?}
\end{figure}
\newdo{this is still no description of the second plot. you're only sneaking this into the back of the findings. make it explicit and move it to the front. }
\newtext{The increasing variance with temperature is camoflauged in the Figure \ref{example1} due to the changes in slope, but is very obvious in the residual plot in Figure \ref{example2} once the trend has been removed.}

The sine illusion (also known in graphical literature as the line width illusion) is a frequent occurrence in statistical analysis, and graphical displays should be thoughtfully considered to minimize its effect visually and acknowledge its influence. Figure \ref{original} shows the classical sine illusion. The parameters that influence the sine illusion are the amplitude of the function and the line length. 
\newdo{how do amplitude and line length impact the illusion - just put one sentence here.}
...
\caption{\label{original}The original Sine Illusion. The regions of the left graph in the peak and the valley appear to be composed of longer lines than the other regions. }
...
\end{figure}
The first documentation we could find of this illusion, \cite{day:1991} documented the illusion in the context of sinusoidal vertical lines. More recently the illusions have been documented on non-sinusoidal curves \citep{schonlau:2003, marie}, but the underlying effect seems to be the same, \newtext{that is, the illusion does not rely on the periodic nature of the underlying trendline, as demonstrated in Figure \ref{twoillusions-simulation}. The ``line width illusion" and the ``sine illusion" are the same phenomenon, even though the psychological literature and the graphics literature refer to the illusion differently.} 
...
\begin{subfigure}[b]{.31\linewidth}\centering
  ...
  \label{simulation3}
\end{subfigure}
\caption{(a) shows seasonality without any underlying trend, (b) shows seasonality superimposed on a quadratic trend, and (c) shows a quadratic trend without seasonality. Though all three sets of simulated data have constant variance, none of the variances appear constant due to the sine illusion.}\label{twoillusions-simulation}
\end{figure}
...
The illusion demonstrated in Figures \ref{exampleFull1} and \ref{original} has been frequently noted in statistical graphics, though usually not as an optical illusion. Rather, the problem is typically identified as the difficulty of visually subtracting two curves, and the misleading conclusions that may result when this process goes awry. Figure \ref{playfair-debt} shows one of the possibly oldest example of this common phenomenon \citep{playfair, playfair2}: the vertical distance between the two lines is much less visually salient than the extant width of the line, that is, the width of the line segment which is perpendicular to the tangent line at a given point.
...
\begin{figure}[h!tbp]
\centering
\includegraphics[width=.5\linewidth]{figure/fig-effectivegraphsfig}
\caption{A replication of Figure 2.16 in \cite{robbins:2005}. The line width illusion seems to be related to increasing slope - as the slope increases, the apparent distance between the two lines decreases, even though the lines represent a constant vertical shift.}
\label{robbins-graph}
\end{figure}

\cite{cleveland:1984, cleveland:1985} also discuss the perceptual difficulty of judging the difference between two curves plotted on the same axis. They recommend the difference between the two curves as an alternate display, as shown in the second part of Figure \ref{cleveland-figure}. While this is good practice when there are two separate curves (as in Figure \ref{playfair-debt}), when the contours of the data implicitly create two curves (the boundaries of the variability in the data) there is not an easy way to display the difference between these implicit curves without separating the display of the trendline and the variance, which creates a more complex set of graphs that require more cognitive effort to interpret. %In addition, as in Figure \ref{playfair-debt} both the difference between the curves and the overall trend are of visual importance, and plotting the difference between the two curves removes the overall trend. 
% 
% \begin{figure}[h!tbp]
% \centering
...
One illuminating demonstration of the Sine illusion comes from M. Bach's applet \citep{bach}. The applet cites perception research suggesting that the sine illusion occurs due to misapplication of perceptual experience with the three-dimensional world to a two-dimensional ``artificial" display of data. One possible three-dimensional context for similar stimuli is shown in Figure \ref{ribbon-illusion}. 

\begin{figure}[h!tbp]\centering
<<ribbon-illusion,echo=FALSE, out.width='0.5\\linewidth', fig.width=5, fig.height=2.6>>=
angle <- .02
shift <- pi/30
...
qplot(data=persp, x=xtop, xend=xbottom, y=ytop, yend=ybottom, geom="segment") + 
  xlab("") + ylab("") + 
  theme(panel.grid.major=element_blank(), 
        panel.background = element_rect(fill = "white", colour = "black"),
        panel.grid.minor=element_blank(), panel.background=element_blank(),
...
\caption{What the brain expects the data to look like. Experience with real-world objects suggests that the data are most similar to a slightly angled top view of a 3-dimensional strip such as a ribbon. Even though the sine illusion lines are not angled, the brain often recruits paradigms from one situation when processing novel stimuli. Optical illusions result from the mismatch between the applied heuristic and the stimulus. \citep{goldstein}} % goldstein S&P 8th edition pg 249
\label{ribbon-illusion}
\end{figure}

An interesting facet of the sine illusion is that when the graph is examined as if it were a two-dimensional projection of a three-dimensional figure, the illusion disappears and the line widths seem once more to be constant. In addition, if the lines are considered without the surrounding context, judgments comparing the length of two lines seem to be relatively accurate, as in \citet{cleveland:1984}. The contextual cues of the overall trend are critical to the sine illusion's effect; without those cues (either by subtraction of the trend or by considering only one or two lines at a time) the illusion does not occur.
\todo[inline]{Add shiny server illustration of this effect by blanking out all but 2 lines that can then be compared accurately.}
That is, the illusion breaks down if the visual heuristic is pre-empted by an attempt to view the graph as if it were three dimensional or if each piece is considered without the surrounding context. The illusion then holds only when a significant portion of the graph is considered simultaneously without regard to the perceptual mechanisms at play (See \cite{day:1991} for a more thorough discussion of how much surrounding context is required for the illusion to persist).

<<originalgrid, dependson='data', echo=FALSE, include=FALSE>>=
f <- function(x) 2*sin(x)
fprime <- function(x) 2*cos(x)
f2prime <- function(x) -2*sin(x)
...
\newdo{Figure \ref{OrthogonalWidth} is beautiful and deserves some detailed discussion here. Do not orphan it again.}
...
The visual preference for judging the length of the line by the orthogonal extant suggests that the underlying illusion may be a function of geometry rather than some mysterious visual process that occurs automatically between perception of the stimulus and conscious thought. 

If this is the case, then it may be  possible to correct the graphical display for so that it is not as visually deceptive. 

<<transform-illustration,echo=FALSE, message=FALSE, warning=FALSE, include=FALSE>>=
f <- function(x) 2*sin(x)
fprime <- function(x) 2*cos(x)
f2prime <- function(x) -2*sin(x)
library(plyr)
...
\caption{\label{OrthogonalWidth} Changes in perceived orthogonal width due to the change in secant angle. 
}
\end{figure}

\newtext{
When we judge the variability of data, we must overcome the visual bias due to the sine illusion. As it turns out, this illusion is incredibly difficult to overcome even when we are aware of its presence: our brains simply cannot ``un-see" the illusion. What follows is a compilation of various methods to correct for or mitigate the effect of these two illusions, intended primarily to demonstrate how pervasive the illusion is and the extreme methods necessary to remove its' effect. }
%\todo[inline]{Add in "we do not expect that these methods will be used in practice" here or at the end?}
%\newdo{let's put that to the end - we don't want to take the wind out of the readers' sails now.}

\section{Breaking the Illusion}
As the illusion is caused by a conflict between the vertical length and the width orthogonal to the slope, it can be resolved by making the orthogonal width constant. This can be accomplished in several ways: by separating the trend and the variability, by forcing the slope to be constant (by reparameterizing the $x$ axis), or by making the lines appear to be of the same height by adjusting their lengths to maintain a constant orthogonal width. Each of these approaches is discussed in the following section.

\subsection{Trend Removal}
\newtext{
One approach to breaking the sine illusion is to present the trend and variance separately, as shown in Figure \ref{trendremoval} and commonly recommended \citep{wainer:2000}, particularly for assessing model fit.} A loess smooth was used to estimate the trendline, and this estimate was subtracted from the line center to produce separate graphs of the trend and the variance. While this does largely remove the sine illusion, it also makes it somewhat more difficult to parse the overall data, as judgments must be made by combining two graphs or by mentally re-composing the original graph (at which point, the sine-illusion becomes a factor). Ideally, it would be possible to tell the whole story with a single graph, to minimize cognitive demands. While this example is relatively simple, it would be much harder to create a de-trended version of Figure \ref{playfair-debt} that could be easily assessed to determine both trend and change in the size of the trade imbalance over time. 
...
\end{subfigure}
\caption{Separating the trend and the variability removes the Sine illusion. Figure \ref{trendremovalFit} shows the fit line compared to the original function, Figure \ref{trendremovalVariability} shows the the underlying fitted trendline and the leftover variability in the vertical lines which now appear to be of the same size.}\label{trendremoval}
\end{figure}

From a cognitive perspective, separating out the trendline and the variance creates more of a barrier to understanding the data in its original form. Additionally, removing the trend requires an initial model, and any plots produced using that fit are conditional on the assumptions made to obtain that model fit. In many situations, this may be undesireable, particularly as we typically view the data before fitting even a rudimentary model, and the sine illusion may influence even initial modeling decisions.
...
\newtext{Let us assume that we can describe the relationship between variables $X$ and $Y$ by a model of the form 
\[
y = f(x) + \varepsilon,
\]
...
The idea of correcting for the illusion is to make all lines appear under the same slope, i.e. we want to find a transformation $\tilde{x}$ of $x$, such that $f(\tilde{x})$ is a  piece-wise linear function, where each piece has the same absolute slope. 
As the sine illusion depends on changing slope in the overall trend, by reparameterizing the $x$-axis in terms of the slope, the data seem to have constant slope, removing the illusion, while the $x$-axis shows the changing slope.
Since the slope is determined by the aspect ratio, we are free to choose it and w.l.o.g. we get for each piece $T_i$: 
\[
f(T_i(x)) = \pm a x + b_i.
\]
This means that $T_i$ is essentially an inverse of function $f$, with each piece defined by the   intervals on which the inverse of $f$ exists: let ${x_0, ..., x_K}$ be the set of values, for which the derivative of $f$ is zero, i.e. $f^\prime(x_i) = 0$ for all $i = 0, ... , K$. 
...
Let function $s = \s{[a,b]}{[c,d]}$ be the linear scaling function that maps  the interval $(a,b)$ linearly to the interval $(c,d)$. This function is defined as
\[
s(x) = \s{[a,b]}{[c,d]} (x) = (x-a)/(b-a) \cdot (d-c) + c.
\]
...
Two scaling functions can be evaluated one after the other, only if the image of the first coincides with the range of the second. Then they form another linear scaling: 
\[
...
\[
\s{[c,d]}{\range{f([x_{i-1}, x_i])}}(x) = f(x_{i-1}) + (x-c)/(d-c) \cdot (f(x_{i}) - f(x_{i-1}))).
\]
...
T_i(x) &=& f^{-1}\left( \s{[c_i,d_i]}{\range{f([x_{i-1}, x_i])}}(x) \right). 
\end{eqnarray}
%
Using this definition for the transformation makes $f(T(x))$ a piece-wise linear function, with parameters $c_i$ and $d_i$ determining the speed, i.e. for $x \in (c_i,d_i)$ we have
\[
f(T(x)) = f (f^{-1}(\s{[c_i,d_i]}{\range{f([x_{i-1}, x_i])}}(x))) = \s{[c_i,d_i]}{\range{f([x_{i-1}, x_i])}}(x).
\]
Correspondingly, the slope of $f(T_i(x))$ is $(f(x_{i}) - f(x_{i-1})))/(d_i-c_i)$.
In order to make the slope the same on all pieces $T_i$, we need to define $c_i$ and $d_i$ with respect to the function values on the interval $(x_{i-1}, x_i)$. There are various options, depending on how closely our $x$-range should reflect the original range.
\begin{itemize}
\item for $[c_i, d_i] = \range(f(x))$ the new $x$-range is the range of $f(y)$, but the scaling function simplifies to the identity.

\item in order to preserve the original $x$-range, we can choose $[c_i, d_i]$ as .... . 
\end{itemize}

\newdo{this needs to be re-written in terms of $s$, $c_i$ and $d_i$s - but it's the same idea.}
That is, if the $x$-axis spans the interval $[a,b)$, the transformation of the $x$ axis is 
$$T(x) = a + (b-a)\left(\int_a^x |f^\prime(z)| dz\right)/\left(\int_a^b|f^\prime(z)| dz\right)$$ 

The transformation is shown in Figure \ref{xtrans1}. An analysis of $T(x)$ is shown in appendix \ref{XtransAppendix}.

In practice we can see that the illusion can be broken by a much less severe transformation in $x$. 
\newtext{By shrinking the transformation in Figure \ref{xtrans1} towards the original $x$ values by a factor of $w$, the illusion is still broken and the data are closer to the original function $y=f(x)$. 
$$T(x) = \frac{w-1}{w} x + \frac{(b-a)}{w}\left(\int_a^x |f^\prime(z)| dz\right)/\left(\int_a^b|f^\prime(z) dz|\right)$$ }

The extent to which this weighting holds is likely to vary with the aspect ratio and the shape of the function, but merits further investigation, as it may be possible to find an ``optimal weighting" for specific situations which minimizes distortion and maximizes the correspondence between inferences made from the data and inferences made using the visual display.\newtext{Figures \ref{xtrans2} - \ref{xtrans4} show the effect of different shrinkage coefficients $w$.} 
...
dots$x2 <- correctx(seq(0, 2*pi, pi/8), fprime, shrink=1)

dframe$xtrans2 <- correctx(dframe$x, fprime=fprime, shrink=1)
...
dots$x3 <- correctx(seq(0, 2*pi, pi/8), fprime, shrink=2)

dframe$xtrans3 <- correctx(dframe$x, fprime=fprime, shrink=2)
...
dots$x4 <- correctx(seq(0, 2*pi, pi/8), fprime, shrink=3)

dframe$xtrans4 <- correctx(dframe$x, fprime=fprime, shrink=3)
...
\caption{$X$ axis transformation}
\label{xtrans1}
\end{subfigure}
\begin{subfigure}[b]{.48\linewidth}\centering
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{figure/fig-xtransform2}
\caption{Weighted Transformation, $w=1$}
\label{xtrans2}
\end{subfigure}

\begin{subfigure}[b]{.48\linewidth}\centering
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{figure/fig-xtransform3}
\caption{Weighted Transformation, $w=2$}
\label{xtrans3}
\end{subfigure}
\begin{subfigure}[b]{.48\linewidth}\centering
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{figure/fig-xtransform4}
\caption{Weighted Transformation, $w=3$}
\label{xtrans4}
\end{subfigure}
\caption{$X$ axis transformations. Dots at the bottom of the graph show the transformation's effect on equally spaced points along the $x$-axis. In (a), adjusting the $x$-spacing of the lines mitigates the sine illusion by changing the extant width so that it is uniform in $x$. (b) - (d) shrink the transformation in (a) towards successively more uniform spacings in $x$ while still breaking the effects of the illusion.}
\label{xtrans}
\end{figure}
\newdo{add a 2nd example here - how about a correction for the ozone data from the beginning?}
\clearpage

\subsection{Transformation of the $Y$-Axis}
Understanding the geometry of the sine illusion leads to another approach to disrupting the conflict between the extant width and the actual height of the segment. 

All of our approaches in this section are based on a comparison of the orthogonal width to the vertical width of lines. 
...
<<geometry,echo=FALSE, fig.height=3, fig.width=5, include=FALSE>>=
...
\includegraphics[width=.5\linewidth]{figure/fig-geometry}
\end{center}
\caption{\label{fig:linewidth}Sketch of line width assessments: The solid line shows vertical width, the dotted line shows  width orthogonal to the slope. }
\end{figure}
% 

As sketch \ref{fig:linewidth} shows, orthogonal $w_o$ and vertical $w_v$ line widths are related -- the orthogonal line width depends on the angle under which the line is drawn. Mathematically, we have:
...
fprime <- function(x) 2*cos(x)
f2prime <- function(x) -2*sin(x)
df <- createSine(40 , len=1, f=f, fprime=fprime)[5:16,]
a <- 9
...
qplot(data=df.real, x=x, y=y, geom="line") +
  ...
  geom_text(aes(x=.5*dfsec$sec.xstart+ .5*dfsec$x +.05 , y=.5*dfsec$sec.ystart+.5*dfsec$y-0.03), label="paste(frac(m, 2))", parse=TRUE, face="plain", data=data.frame()) + 
  xlab("x") + ylab("y") +
...
dev.off()
# ggplot(data=df, aes(x=xstart, xend=xend, y=ystart, yend=yend)) + theme_bw() + coord_fixed(ratio=1) + 
#   geom_segment(colour="grey30") + 
#   geom_segment(data=df[a,], aes(x=xend-.008, xend=x-.008, y=yend, yend=y), colour="grey50") + 
#   geom_point(aes(x=x, y=y)) + 
#   geom_segment(data=df[a,], aes(x=xstart, xend=xend, y=ystart, yend=yend)) + 
#   geom_segment(data=dfsec, aes(x=sec.xstart+.005, xend=x+.005, y=sec.ystart-.005, yend=y-.005), colour="grey50") + 
#   geom_segment(data=dfsec, aes(x=sec.xstart, xend=sec.xend, y=sec.ystart, yend=sec.yend)) + 
# 
#   geom_segment(data=dfslope, linetype=2) + 
#   geom_segment(data=dfslope2, linetype=2) + 
#   geom_text(data=dfslope, aes(x=xend+.1, y=yend+.05, label="slope: paste(f, \"'\", (x))"), parse=TRUE, hjust=0, vjust=1) +
#   geom_text(aes(x=mean(c(dfsec$sec.xstart, df$xend[a], df$x[a], df$x[a])), 
#                 y=mean(c(dfsec$sec.ystart, df$yend[a], df$y[a], df$y[a])), 
#                 label="theta"), parse=TRUE, size=ftsize) +
# #   geom_text(aes(x=dfslope$xend,  y=dfslope$yend+.075, 
# #                 label=paste("paste(y == f, \"'\", (x[0]) (x-x[0]) + f(x[0]))")), 
# #             parse=TRUE, hjust=.25, size=ftsize) + 
#   geom_text(data=df[a,], aes(x=.5*xend-.03 + .5*x, y=.5*yend+.5*y, label="\u2113"), size=ftsize+1, face="bold") +
#   geom_text(aes(x=.5*dfsec$sec.xstart+ .5*dfsec$x +.05 , y=.5*dfsec$sec.ystart+.5*dfsec$y-0.05, label="italic(m)"), parse=TRUE, face="plain") + 
#   xlab("x") + ylab("y") 

@
\begin{figure}[h!tbp]\centering
\includegraphics[keepaspectratio=TRUE,width=.5\linewidth]{figure/fig-trigmethodcartoon}
\caption{\label{fig:trig} Geometry of the trigonometry correction approach. $\ell$ denotes the length of the entire vertical segment, hence, $\ell/2$ is the length of the portion above the mean function.
\newdo{{\tt theme\_minimal} is not defined; there was a $1/2$ missing for $m$ -- either we use $\ell$ and $m$ or $\ell/2$ and $m/2$. Mixing those makes the math more complicated.}
}
\end{figure}
% \todo[inline]{
...
\ell_{\text{new}} = \ell\cdot\left(\cos\left[\atan{\left| f^\prime (x) \right|}\right]\right)^{-1}
\end{equation}
...
\end{subfigure}
\caption{Fix of the sine illusion by using the factor of $1/\cos\left(\atan{|\cos(x)|}\right)$} to adjust the vertical extent of the lines.  
\label{trigFix} 
\end{figure}

\newdo{Figure \ref{trigFix} deserves a mentioning here.}
...
\newtext{The correction factor seems to over-correct. Is this just something that happens to the trained eye? We might need people to test for it.}

\subsubsection{A General Correction Method}
The idea for the general correction method is sketched out in figure \ref{GeneralCorrection}. Based on function $f$ that describes the overall trend of the relationship between $X$ and $Y$ we want to first want to find the perceived width, and then correct the vertical width accordingly to match with the audience's expectation.

\begin{figure}[hbtp]
\centering
\includegraphics[keepaspectratio=TRUE,width=.5\linewidth]{figure/fig-generalcorrectioncartoon1}
\caption{\label{GeneralCorrection}
General correction approach. This approach may require numerical optimization to obtain exact solutions for $(x_u, y_u)$ and $(x_l, y_l)$.}
\end{figure}
...
qplot(data=df.real, x=x, y=y, geom="line") + 
  ...
  coord_equal(ratio=1) + theme(axis.title=element_blank(), axis.text=element_blank(), axis.ticks=element_blank(), axis.line=element_blank(), panel.grid=element_blank())
...
\begin{subfigure}[a]{0.49\linewidth}\centering
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{figure/fig-generalcorrectioncartoon2}
\caption{Linear Approximation}\label{linear-GeneralCorrection}
\end{subfigure}
\begin{subfigure}[b]{0.49\linewidth}\centering
\includegraphics[keepaspectratio=TRUE,width=\linewidth]{figure/fig-generalcorrectioncartoon3}
\caption{Quadratic Approximation}\label{quadratic-GeneralCorrection}
\end{subfigure}
\caption{\label{linear.quadratic} (a) uses a first-order taylor series approximation to $f(x)$ and (b) uses a second-order taylor series approximation to $f(x)$. The intersection of the function $f(x) \pm \ell/2$ and the secant line,  $(x_u, y_u), (x_l, y_l)$ must be obtained to determine the necessary correction factor.}
\end{figure}
...
\end{subfigure}
\begin{subfigure}[b]{.45\linewidth}\centering
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-trigFix3}
\caption{Correction factor in Eqn. \ref{LinearApprox}}
\end{subfigure}
\caption{Fix of the sine illusion by using the factor of $\sqrt{1 + f^\prime(x)^2}$} to adjust the vertical extent of the lines.  
\label{GeneralLinearCorrection} 
\end{figure}

Figure \ref{GeneralLinearCorrection} shows the correction factor compared with the trigonometry-style correction and the untransformed data. 
...
\[
\lambda_{1/2} = \pm \ell \cdot \left( \sqrt{ (f^\prime(x_0)^2 + 1)^2 -  f^{\prime\prime}(x_0)  f^\prime(x_0)^2 \cdot (\pm \ell)} + (f^\prime(x_0)^2 + 1) \right)^{-1}
\]

% \begin{eqnarray*}
% \left(  f^{\prime\prime}(x_0)  f^\prime(x_0)^2 \right)^{-1} \left( -b + a \right) = \\
% \left(  f^{\prime\prime}(x_0)  f^\prime(x_0)^2 \right)^{-1} \left( -b^2 + a^2 \right) \left( a + b \right)^{-1} = \\
%  \pm \ell \cdot \left( \sqrt{ (f^\prime(x_0)^2 + 1)^2 -  f^{\prime\prime}(x_0)  f^\prime(x_0)^2 \cdot (\pm \ell)} + (f^\prime(x_0)^2 + 1) \right)^{-1} = \\
% \end{eqnarray*}
% 
% As discussed before, solutions do not exist for $f^\prime(x) = 0$ and $f^{\prime\prime}(x) = 0$. But we also need to take special care in situations where we come close to those values. We do this by using L'Hospital's rule:
% 
% For $h = f^{\prime\prime}(x)$ we have
% \begin{eqnarray*}
% \lim_{h \rightarrow 0} \left(  h  f^\prime(x)^2 \right)^{-1} \left( -(f^\prime(x)^2 + 1) \pm \sqrt{ (f^\prime(x)^2 + 1)^2 -  h  f^\prime(x)^2 \cdot (\pm \ell)} \right) &\stackrel{\text{L'Hosp.}}{=}& \\
% \lim_{h \rightarrow 0} \left(   f^\prime(x)^2 \right)^{-1} \left(  (\pm 0.5) \cdot \left( (f^\prime(x)^2 + 1)^2 -  h  f^\prime(x)^2 \cdot (\pm \ell) \right)^{-1/2} \cdot \left( - f^\prime(x)^2 \cdot (\pm \ell) \right) \right) &=&\\
% \left(   f^\prime(x)^2 \right)^{-1} \left(  (\pm 0.5) \cdot \left( f^\prime(x)^2 + 1 \right) \cdot \left( - f^\prime(x)^2 \cdot (\pm \ell) \right) \right) &=& \\
% \pm 0.5 \cdot \left( f^\prime(x)^2 + 1 \right) \cdot  \ell   &=& \lambda.
% \end{eqnarray*}
% %
% For $h = f^{\prime}(x)^2$ the limit is
% \begin{eqnarray*}
% \lim_{h \rightarrow 0} \left(  f^{\prime\prime}(x)  \cdot h \right)^{-1} \left( -(h + 1) \pm \sqrt{ (h + 1)^2 -  f^{\prime\prime}(x)  \cdot h \cdot (\pm \ell)} \right)
% &\stackrel{\text{L'Hosp.}}{=}& \\
% \lim_{h \rightarrow 0} \left(  f^{\prime\prime}(x)  \right)^{-1} \left( -1 \pm 0.5 \cdot \left( (h + 1)^2 -  f^{\prime\prime}(x)  \cdot h \cdot (\pm \ell) \right)^{-1/2} \cdot \left( 2(h + 1) -  f^{\prime\prime}(x) \cdot (\pm \ell) \right) \right) &=&\\
%  \left(  f^{\prime\prime}(x)  \right)^{-1} \left( -1 \pm  \left( 1 -  f^{\prime\prime}(x) \cdot (\pm \ell/2) \right) \right) &=& \lambda.\\
%  \end{eqnarray*}
% \begin{eqnarray*}
% \lim_{f^{\prime}(x)^2 \rightarrow 0} \left(  f^{\prime\prime}(x)  f^\prime(x)^2 \right)^{-1} \left( -(f^\prime(x)^2 + 1) \pm \sqrt{ (f^\prime(x)^2 + 1)^2 -  f^{\prime\prime}(x)  f^\prime(x)^2 \cdot (\pm \ell)} \right) = (L'Hosp.)\\
% \lim_{f^{\prime}(x)^2 \rightarrow 0} \left(  f^{\prime\prime}(x)  \right)^{-1} \left( -1 \pm 0.5 \left({ (f^\prime(x)^2 + 1)^2 -  f^{\prime\prime}(x)  f^\prime(x)^2 \cdot (\pm \ell)}\right)^{-1/2} \cdot \left( 2\cdot (f^\prime(x)^2 + 1) -  f^{\prime\prime}(x)  \cdot (\pm \ell)\right) \right) 
% = \\
%  \left(  f^{\prime\prime}(x)  \right)^{-1} \left( -1 \pm 0.5\right)  \cdot \left( 2 -  f^{\prime\prime}(x)  \cdot (\pm \ell)\right) 
% = 
% \end{eqnarray*}

As shown in Figure \ref{quadratic-GeneralCorrection}, the solution of this equation provides endpoints $(x_l, y_l)$ and $(x_u, y_u)$ which can then be used to compute the extant width of each half-segment $m$. Unlike the linear solution, the half-segments here are not necessarily of the same length, and thus there are separate correction factors for each half-segment. 
...
dframe <- createSine(n = 40, len = 1, f=f, fprime=fprime)
dframe$type <- "Data"
dframe.real <- createSine(n = 4000, len = 1, f=f, fprime=fprime, getquadapprox=TRUE, f2prime=f2prime)
secantlines <- do.call("rbind", lapply(dframe.real$x, function(i) getSecantSegment(i, dframe, f, fprime, f2prime)))
# secantlines2 <- getAdjLength(secantlines)

dframe2 <- secantlines
names(dframe2) <- c("x", "y", "deriv", "xstart", "xend", "ystart", "yend", "ell", "ell1", "ell2", "type", "a")
...
cairo_pdf("figure/fig-wtf.pdf", width=6, height=6)
ggplot(aes(x=x, y=len, linetype=Line), data=secantlines.long) + geom_line()+
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), 
                     labels=c("0", expression(paste(pi,"/2")), 
                              ...
                              expression(paste("2",pi)))) + 
  theme_bw() + theme(legend.position="bottom") + scale_linetype_discrete("Adjusted \u2113/2")
dev.off()
@
\begin{figure}[h!tbp]\centering
\includegraphics[keepaspectratio=TRUE, width=.8\linewidth]{figure/fig-wtf}
\caption{Extant Half-segment Length. The secant  line segments are not symmetrical around the center line, so symmetric correction as employed in Figure \ref{GeneralLinearCorrection} leads to simultaneous over- and under-correcting.}\label{HalfSegLength}
\todo[inline]{Something's strange here - minimum length should be 0.5, not 0.4, and the aspect ratio wouldn't account for that discrepancy. I'm pretty sure if I can hunt this bug down, it might fix most of what's wrong.}
\end{figure}

\begin{figure}[h!tbp]\centering
<<CtsAdjustedResults,echo=FALSE, dependson='CtsQuadratic'>>=
dframe <- createSine(n = 40, len = 1, f=f, fprime=fprime, getquadapprox=TRUE, f2prime=f2prime) 
qplot(x=x, xend=x, y=y+ellx4.u, yend=y-ellx4.l, geom="segment", data=dframe, linetype=I(1)) + coord_fixed(ratio=1) + theme_bw() + ylab("y") + 
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), 
                     labels=c("0", expression(paste(pi,"/2")), expression(pi), expression(paste("3",pi, "/2")), expression(paste("2",pi))))
@
...
\todo[inline]{Check formula - this doesn't seem quite right...}
\newdo{I'm pretty sure it's the missing correction for the length of the lines. }
\end{figure}
\todo[inline]{Modify formula for aspect ratio - not sure exactly how to do this, need to look at the algebra *again*}
\clearpage
...
\todo[inline]{Discussion: Hammock/Trig breaks first, linear breaks next, quadratic breaks eventually}
As with the trigonometry correction, the linear adjustment is prone to the same overcorrection distortion shown in Figure \ref{linearFixOvercorrect}.


Some modifications are necessary to implement the correction factor in a realistic situation in which it might not be practical for the aspect ratio of the graph to be fixed at 1. As in \todo{Cite Marie's paper}, the correction factor necessary to remove the illusion depends on the aspect ratio of the graphs. Figure \ref{trigFix-aspectRatio} demonstrates the relative effectiveness of the correction factor.
<<aspectratio, echo=FALSE, include=FALSE>>=
f <- function(x) 2*sin(x)
fprime <- function(x) 2*cos(x)
f2prime <- function(x) -2*sin(x)
...
qplot(x=xstart, xend=xend, y = y+ellx/2, yend = y-ellx/2, geom="segment", data=dframe1) + 
  coord_fixed(ratio=.5) + xlab("x") + ylab("y") + theme_bw() + 
#  ylim(c(-pi, pi)) + 
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), 
                     labels=c("0", expression(paste(pi,"/2")), 
                              expression(pi), 
                              ...
                              expression(paste("2",pi))))
qplot(x=xstart, xend=xend, y = y+ellx/2, yend = y-ellx/2, geom="segment", data=dframe1) + 
  coord_fixed(ratio=1) + xlab("x") + ylab("y") + theme_bw() + 
#  ylim(c(-pi, pi)) + 
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), 
                     labels=c("0", expression(paste(pi,"/2")), 
                              expression(pi), 
                              ...
                              expression(paste("2",pi))))
qplot(x=xstart, xend=xend, y = y+ellx/2, yend = y-ellx/2, geom="segment", data=dframe1) + 
  coord_fixed(ratio=2) + xlab("x") + ylab("y") + theme_bw() + 
#  ylim(c(-pi, pi)) + 
  scale_x_continuous(breaks=seq(0, 2*pi, by=pi/2), 
                     labels=c("0", expression(paste(pi,"/2")), 
                              expression(pi), 
                              expression(paste("3",pi, "/2")), 
                              expression(paste("2",pi))))
# qplot(x=xstart, xend=xend, y = y+ellx/2, yend = y-ellx/2, geom="segment", data=dframe1) + ylim(c(-2*pi, 2*pi))+ coord_fixed(ratio=2)+ xlab("x") + ylab("y") + theme_bw()+ 
#   scale_x_continuous(breaks=seq(0, 2*pi, by=pi), 
#                      labels=c("0", expression(pi), expression(paste("2",pi))))
# qplot(x=xstart, xend=xend, y = y+ellx/2, yend = y-ellx/2, geom="segment",data=dframe1) + ylim(c(-4*pi, 4*pi))+ coord_fixed(ratio=2)+ xlab("x") + ylab("y") + theme_bw()+ 
#   scale_x_continuous(breaks=seq(0, 2*pi, by=pi), 
#                      labels=c("0", expression(pi), expression(paste("2",pi))))
@
\begin{figure}[h!tbp]\centering
\begin{subfigure}[b]{.32\linewidth}\centering
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-aspectratio1}
\caption{Aspect Ratio of .5}
\end{subfigure}
\begin{subfigure}[b]{.32\linewidth}\centering
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-aspectratio2}
\caption{Aspect Ratio of 1}
\end{subfigure}
\begin{subfigure}[b]{.32\linewidth}\centering
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-aspectratio3}
\caption{Aspect Ratio of 2}
\end{subfigure}
\caption{Changing aspect ratios changes the strength of the illusion and the effectiveness of the transformation}\label{trigFix-aspectRatio} 
\end{figure}
...
ggplot()  + geom_jitter(aes(Time, value/100), data=subset(ushcn, Year>1995), size=1) + 
  ...
  ylab("Temperature (in Celsius)") + theme_bw()
# ggsave(file="figure/monthly-temps.pdf", width=12, height=4.5)
@
% \includegraphics[width=\textwidth]{monthly-temps}
\caption{\label{raw-temp}Raw monthly maximum temperatures (in Celsius) between 1980 and 2012 at 422 stations across the US with similar elevation and latitude. The blue line shows fitted temperatures from a baseline model. The plot suggests that the model fails to capture the more extreme values in summer and winter. }
\end{figure}
...
ggplot()  + geom_boxplot(aes(Time, value/100, group=interaction(Year, Month)), outlier.size=1, fill = "grey65", data=subset(ushcn, Year>1995)) + theme_bw() + 
  ...
  ylab("Temperature (in Celsius)")+ xlab("Time")
# ggsave(file="figure/monthly-temps.pdf", width=12, height=4.5)
@

\caption{\label{box-temp}Monthly boxplot of maximum temperatures (in Celsius) between 1980 and 1995 at 422 stations across the US with similar elevation and latitude. The boxplots suggest that there is more variation in the winter months than in the other seasons. Summer, fall and spring appear to have similar temperature variability, and there are more outliers in the summer.}\label{boxplot-temp}
\end{figure}
\begin{figure}[hbtp]
\centering
...
\caption{\label{resid-temp}Residual monthly temperatures (in Celsius) from the baseline model fit.}
\end{figure}
...
# ggsave(file="figure/monthly-resid-temps.pdf", width=12, height=4.5)
@
\caption{Boxplots of the residuals provide more insight into the seasonal variance: Winters show more variation in temperatures, while summers show much less variation in temperatures. Our initial impression from the boxplots of the raw temperature data was only partially correct - while winters certainly  have higher variation, summers show less variation than the fall and spring, albeit with more outliers.}\label{boxplot-resid-temp}
\end{figure}
\todo[inline]{Add Citations for USHCN website, data, etc.}
Figures \ref{raw-temp} and \ref{resid-temp} show temperature data from 422 stations across the US with similar elevation and latitude. The first plot shows raw temperatures, the second residuals from a `baseline' model. The caption for the raw temperature plot suggests that the baseline model is not quite able to capture all of the variability in summer and winter peaks. However, the model used is actually based on averages in each time point. This model is not  susceptible to shrinking values, and it is our perception of the plot rather than the data that makes us see any higher variability in the peaks. After removing the averages in Figure \ref{boxplot-temp} to yield Figure \ref{boxplot-resid-temp} the impression of higher variability in the peaks is only partially confirmed: there is higher variability in the winter, and lower variability in the summer, though there are more outliers in the summer than in the winter.

\clearpage
\section{Conclusion}
\begin{itemize}\item Raise Awareness\item ``Fixing" data is tricky and usually beyond a statistician's comfort zone, but we could see this as part of an applet where the ``fix" is done temporarily as a diagnostic measure\end{itemize}
\clearpage
...
\clearpage
\appendix
\section{X Transformation Math}\label{XtransAppendix}
Suppose that 
\begin{itemize}
\item $f(x)$ is defined on $[a,b)$
\item $f^\prime(x)$ exists on $[a,b)$
\item countable regions $[a_i, a_{i+1})$ exist such that $\bigcup_{i=1}^\infty [a_i, a_{i+1}) = [a,b)$ and either $f^\prime(x) \geq 0$ or $f^\prime(x) \leq 0$ for all $x\in [a_i, a_{i+1})$.
\end{itemize}
Then for $$T(x) = \frac{(a_\infty-a_0)}{\int_{a_0}^{a_\infty} |f^\prime(z)| dz} \int_{a_0}^x|f^\prime(z)| dz + a$$
\begin{align*}
T(x) & = a + \frac{a_{\infty}-a_{0}}{\int_{a_0}^{a_{\infty}} |f^\prime(z)| dz}\left(\sum_{i=1}^\infty \left|\int_{a_i}^{a_{i+1}} f^\prime(z) I(z<x) dz \right|\right)\\
& = a + \frac{a_{\infty}-a_{0}}{\sum_{i=1}^{M} \left|\int_{a_i}^{a_{i+1}} f^\prime(z) I(z<x)\right| dz}\left(\sum_{i=1}^{M-1} \left|f(a_{i+1}) -  f(a_i)\right| + \left|f(x) - f(a_{M}) \right|\right)\\
&\qquad\qquad\text{where } a_{M} < x < a_{M+1}
\end{align*}
So on any single $[a_i, a_{i+1})$, assuming WLOG $x<x_0$
\begin{align*}
T_i(x) & = c_{i1} + c_{i2}|f(x) - f(a_i)|\\
f^\prime_i(x) & = \lim_{x\rightarrow x_0} \frac{f(x) - f(x_0)}{T_i(x) - T_i(x_0)} I(a_i \leq x < x_0 < a_{i+1})\\
& = \lim_{x\rightarrow x_0} \frac{f(x) - f(x_0)}{c_{i1} + c_{i2}\cdot s \cdot (f(x) - f(a_i)) - c_{i1} - c_{i2}\cdot s \cdot(f(x_0) - f(a_i))} I(a_i \leq x < x_0 < a_{i+1})\\
& \qquad\qquad\text{where } s = sign(f^\prime(x)) \text{ on } x\in [a_i, a_{i+1})\\
& = \lim_{x\rightarrow x_0} \frac{f(x) - f(x_0)}{c_{i2}\cdot s \cdot (f(x_0) - f(x))} I(a_i \leq x < x_0 < a_{i+1})\\
& = \lim_{x\rightarrow x_0} \frac{f(x) - f(x_0)}{c_{i2}\left|f(x_0) - f(x)\right|} I(a_i \leq x < x_0 < a_{i+1})\\
& = \frac{1}{c_{i2}}
\end{align*}
Hence $f^\prime(x)$ is constant on $[a_i, a_{i+1})$. 

Furthermore, by definition, $c_{i2}$ is equal to $$c_{i2} = \pm \sum_{i=0}^{\infty} \frac{a_{i+1}-a_i}{a_\infty-a_0}\left(\frac{\left|f(a_{i+1})-f(a_i)\right|}{a_{i+1}-a_i}\right)$$ that is, the weighted average of $|\triangle f(x)|/(\triangle x)$ over the $[a_i, a_{i+1})$ 

Also, $c_{01} = a_0$, $c_{i1} = a_{i+1}-a_i$. Hence the piecewise definition can be made to look like the original definition, but with the $c_{i2}$ differing only by a sign and the cumulative $c_{i1}$ tracking the left endpoint of the segment. 
\end{document}
