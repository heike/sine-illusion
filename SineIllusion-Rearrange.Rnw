...
<<intro-example,echo=FALSE, include=FALSE>>=
library(reshape)
library(plyr)
f <- function(x) 2*sin(4*x/pi)
fprime <- function(x) 2*cos(4*x/pi)
f2prime <- function(x) 2*sin(4*x/pi)
df <- createSine(150, len=1, f, fprime)[,c(1, 4, 7:9)]
df2 <- ddply(df, .(x), transform, ysamp = rnorm(15, y, ellx/2))
qplot(data=df2, x=x, y=ysamp, geom="point", size=I(1.5)) + xlab("x") + ylab("y") + coord_equal(ratio=1) + theme_bw() +ylim(c(-4,4))         
qplot(data=df2, x=x, y=ysamp-y, geom="point") + xlab("x") + ylab("y") + coord_equal(ratio=1) + theme_bw() +ylim(c(-3,3))  
@
<<intro-example-redo,echo=FALSE, include=FALSE>>=
library(reshape)
library(plyr)
f <- function(x) 2*sin(4*x/pi)
fprime <- function(x) 2*cos(4*x/pi)
f2prime <- function(x) 2*sin(4*x/pi)
df <- createSine(150, len=1, f, fprime)[,c(1, 4, 7:9)]
df2 <- ddply(df, .(x), transform, ysamp = rnorm(15, y, mean(df$ellx/2)))
qplot(data=df2, x=x, y=ysamp, geom="point", size=I(1.5)) + xlab("x") + ylab("y") + coord_equal(ratio=1) + theme_bw() +ylim(c(-4,4))     
qplot(data=df2, x=x, y=ysamp-y, geom="point") + xlab("x") + ylab("y") + coord_equal(ratio=1) + theme_bw() +ylim(c(-3,3))  
@
...
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-intro-example1}
\caption{\label{motivation1}}
\end{subfigure}
\begin{subfigure}[b]{.45\linewidth}\centering
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-intro-example-redo1}
\caption{\label{motivation2}}
\end{subfigure}
\caption{Which figure has less heteroskedasticity? }
\end{figure}
...
\centering
\begin{subfigure}[b]{.45\linewidth}\centering
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-intro-example2}
\caption{Residuals from Figure \ref{motivation1}}\label{motivation3}
\end{subfigure}
\begin{subfigure}[b]{.45\linewidth}\centering
\includegraphics[width=\linewidth,keepaspectratio=TRUE]{figure/fig-intro-example-redo2}
\caption{Residuals from Figure \ref{motivation2}}\label{motivation4}
\end{subfigure}
\caption{Figure \ref{motivation2} has constant variance, but appears to have nonconstant variance. Figure \ref{motivation1} has nonconstant variance but appears to have constant variance. }
\end{figure}
...
qplot(x,y, data=dframe, geom="smooth")
qplot(x,y-f(x), data=dframe, geom="jitter") + ylim(c(-3,3))
