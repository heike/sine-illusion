...
\newtext{The goal of the study is to determine an overall `optimal' setting for each of the correction factors. It is of additional interest, whether this setting is  subject-specific or population-based, and how much within-subject variability we find compared to between-subject variability. 
Once a participants arrives at the site, he/she is presented with a specific setting of the weight $w$, where for the first six evaluations $w$ is randomly chosen from the set of numbers $\{0, 1, ...\}$. After that, $w$ is randomly sampled from a normal distribution with mean 0.5 and standard deviation of 0.5. 
}
\subsection{Method}
\begin{figure}
