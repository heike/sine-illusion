...
asp <- ((4)/70)/(10/17) # image vs coordinate aspect ratio 
startyear <- 0
gridlines <- seq(1996, 2013, .125)
...
ushcn.cor.x <- subset(ushcn.cor.x, variable!="Uncorrected")
...
ggplot(aes(x=value, y=temp, group=variable), data=ushcn.cor.x) + 
  geom_jitter(size=1.5, position = position_jitter(width = 1/36), alpha=.75)+ 
  facet_grid(variable~.) +
  geom_line(data=ushcn.cor.x, aes(y=temp.fit), colour="red", size=1.5) + 
...
ushcn.cor.y <- subset(ushcn.cor.y, variable!="Uncorrected")
