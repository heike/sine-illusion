...
<<original-redo, echo=FALSE, fig.keep='none', include=FALSE>>=
...
\begin{figure}[htbp]
\centering
<<transform-illustration,echo=FALSE, include=FALSE>>=
...
<<ribbon-illusion,echo=FALSE, include=FALSE, fig.width=5, fig.height=2.6>>=
library(plyr)
angle <- .02
shift <- pi/30
...
<<ytransform1-plot, echo=FALSE, include=FALSE, fig.width=4, fig.height=c(2, 4, 6)>>=
...
<<xtransform, echo=FALSE, include=TRUE, results='asis', out.width=".31\\textwidth">>=
...
y3end <- sin(x) + .5*l3
@
<<echo=FALSE,include=TRUE,out.width=".48\\textwidth">>=
...
qplot(x=xcomb, y=y3comb,geom="line", main="Corrected by 3a")+ 
  theme(plot.background=element_blank(), panel.grid.major=element_blank(), 
       panel.grid.minor=element_blank(), panel.background=element_blank())+ylim(c(-2, 2))
@
<<echo=FALSE,include=FALSE,out.width=".48\\textwidth">>=
