...
@
\includegraphics[width=.45\linewidth, keepaspectratio=TRUE]{figure/fig-transform-illustration1}
\caption{\label{needslabel} The perceived width orthogonal to the slope differs from the length of the lines. \todo[inline]{include blue orthogonal lines for the whole function. That way we can also see the problem at the changing curvature better. }
}
\end{figure}
...
\begin{figure}[hbtp]
<<fixingit, dependson='data', echo=FALSE, results='asis',out.width='.495\\textwidth', fig.width=5, fig.height=5>>=
...
\todo[inline]{using multicore feels a bit like shooting  on sparrows with cannons.}
