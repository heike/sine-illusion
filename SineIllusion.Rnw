...
qplot(x=x, y=y, geom="line", data=df1, colour=I("red")) + theme_bw() + 
  geom_line(aes(y=y+1), colour="red", linetype=4) + geom_line(aes(y=y-1), colour="red", linetype=4) +
...
x <- seq(-pi, pi, .1)
angle <- .08
persp <- data.frame(xtop=(1-angle)*x, xbottom=(1+angle)*x, ytop = sin((1-angle)*x)+1/2, ybottom = sin(x*(1+angle))-1/2)
...
qplot(data=persp, x=xtop, xend=xbottom, y=ytop, yend=ybottom, geom="segment") + ggtitle("What the brain expects the data to look like") + 
  ...
  theme(panel.grid.major=element_blank(), 
        ...
        axis.text = element_blank()) + coord_equal(ratio=1)
# 
# ribbon <- ldply(x, function(i) data.frame(x = i, y = sin(i+seq(-1, 1, .01)), offset=seq(-1, 1, .01)))
# ribbon2 <- ddply(ribbon, .(x), summarise, lb=min(y), ub=max(y))
# ribbon2$center <- sin(x)
...
# 
# 
# qplot(x=x, y=center, geom="line", data=ribbon2, colour=I("red")) + geom_ribbon(aes(ymax=ub, ymin=lb), colour="blue", alpha=.5) + ggtitle("What our brains expect the data to look like") + 
#   geom_segment(data=df2, aes(x=xstart, xend=xend, y=ystart, yend=yend)) + 
#   xlab("") + ylab("") + 
#   theme(panel.grid.major=element_blank(), 
#         panel.background = element_rect(fill = "white", colour = "black"),
#         panel.grid.minor=element_blank(), panel.background=element_blank(),
...
<<ytransform1-plot, echo=FALSE, fig=FALSE>>=
qplot(x=xstart, xend=xend, y = ystart, yend=yend, colour=I("blue"), geom="segment", main="Transformed Length", data=dframe1) + ylim(c(-pi/2, pi/2))+ coord_fixed() + xlab("x") + ylab("y")
qplot(x=xstart, xend=xend, y = ystart, yend=yend, colour=I("blue"), geom="segment", main="Transformed Length", data=dframe1) + ylim(c(-pi, pi))+ coord_fixed()+ xlab("x") + ylab("y")
qplot(x=xstart, xend=xend, y = ystart, yend=yend, colour=I("blue"), geom="segment", main="Transformed Length", data=dframe1) + ylim(c(-2*pi, 2*pi))+ coord_fixed()+ xlab("x") + ylab("y")
@

\subfigure[]{\includegraphics[width=.32\linewidth,keepaspectratio=TRUE]{figure/fig-ytransform1-plot1}}
\subfigure[]{\includegraphics[width=.32\linewidth,keepaspectratio=TRUE]{figure/fig-ytransform1-plot2}}
\subfigure[]{\includegraphics[width=.32\linewidth,keepaspectratio=TRUE]{figure/fig-ytransform1-plot3}}
