...
\newcommand{\done}[2][inline]{\todo[color=green, #1]{#2}}  % for todos that have been seen and dealt with
\newcommand{\meh}[2][inline]{\todo[color=white, #1]{#2}}   % for todos that may no longer be relevant 
\newcommand{\comment}[2][inline]{\todo[color=cyan, #1]{#2}} % for comments that may not be "to-do"s
...
qplot(x=xstart, xend=xend, y = ystart, yend=yend, geom="segment", data=df1, colour=I("grey20")) +
  ...
  geom_segment(aes(x=x, xend=xend, y=y, yend=yend), colour="grey70", 
  ...
  geom_text(aes(x=df1$x[19], y=df1$y[20], label="\u2113"), hjust=1, vjust=.5, size=10, colour="grey70")
dev.off()
@
...
\done{Should we make the $w_o$ vs. b and $w_v$ vs. a terminology consistent? -- I'm not sure that we keep the figure - let's worry about content first. I like the next figure actually better, but it's not labelled at all yet. HH}
\begin{figure}[htbp]
\begin{center}
