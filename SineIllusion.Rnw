...
qplot(x=x, xend=x, y=0, yend=ell1, geom="segment", data=secantlines, linetype=I(1)) + 
  geom_segment(aes(yend=-ell2), linetype=4) + ggtitle("Extant Segment Length (Centered)") + coord_fixed(ratio=1)
...
qplot(x=x, xend=xend, y=ystart, yend=yend, geom="segment", data=adjY(dframe))+
  theme_bw() + coord_fixed(ratio=1)+ 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), data=secantlines2, colour="blue")  + coord_fixed(ratio=1)
