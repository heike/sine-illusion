...
<<setup, fig.keep='all', cache=FALSE, eval=TRUE>>=
#rm(list=ls())
...
\todo[inline]{Put labels for reference for all of your charts and write a couple of sentences of why you're drawing this chart and what you see ... it doesn't need to stay in the paper necessarily, but it's usually a good start for keeping track of what we want to do.}
...
<<original, echo=FALSE, results='asis', fig.width=5, fig.height=2.6>>=
l <- 1
...
<<original-redo, echo=FALSE, fig.keep='none', fig.keep='all'>>=
cairo_pdf("./figure/fig-original-redo.pdf", width=5, height=2.6)
l <- 1
...
\begin{figure}[hbtp]
\centering
<<originalgrid, dependson='data', echo=FALSE, results='asis',out.width=".5\\linewidth", fig.width=5, fig.height=2.6>>=
...
@
\caption{\label{firstfix} First fix of the sine illusion by using the factor of $1/\cos(\atan(|f^\prime(x)|))$ (middle) and $\sqrt{1 + f^\prime(x)^2}$ (right) for adjusting the vertical extent of the lines.  }
\end{figure}
...
<<boxtemp, dependson='readdata', echo=FALSE, echo=FALSE, fig.width=12, fig.height=4.5, out.width='\\textwidth',cache=TRUE>>=
