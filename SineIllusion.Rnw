...
\todo[inline]{Once you've dealt with a comment, could you color it differently or comment it out, so I can see that you read it and dealt with it? That will help streamline things a bit ... Thanks!}
\todo[inline]{
Put labels for reference for all of your charts and write a couple of sentences of why you're drawing this chart and what you see ... it doesn't need to stay in the paper necessarily, but it's usually a good start for keeping track of what we want to do.}
...
qplot(x=xstart, xend=xend, y = ystart, yend=yend, geom="segment", data=df1, colour=I("grey20")) +
  theme(panel.grid.major=element_blank(), 
        ...
        axis.title = element_blank(), axis.ticks = element_blank(), 
        axis.text = element_blank()) + coord_equal(ratio=1) + 
  geom_segment(x=-pi/12, xend=2*pi+pi/12, y=0, yend=0, colour="grey", linetype=2)+
  geom_segment(aes(x=x, xend=xend, y=y, yend=yend), colour="red", 
  ...
  geom_text(aes(x=df1$x[5], y=df1$y[6]/2, label="s"), hjust=1, vjust=.5, size=10, colour="red") + 
  geom_segment(aes(x=x, xend=xend, y=y, yend=yend), colour="blue", 
  ...
  geom_text(aes(x=df1$x[19], y=df1$y[20], label="\u2113"), hjust=1, vjust=.5, size=10, colour="blue")
dev.off()
@
\includegraphics[keepaspectratio=true, width=\linewidth]{fig-original-redo.pdf}
\end{minipage}
\caption{\label{original}The original Sine Illusion. The regions of the graph in the peak and the valley appear to be composed of longer lines than the other regions. $s$ denotes the amplitude of the function, $\ell$ the length of the line segments. \todo[inline]{We need to mention that there's a left and a right chart.
...
\todo[inline]{Should we make the $w_o$ vs. b and $w_v$ vs. a terminology consistent? -- I'm not sure that we keep the figure - let's worry about content first. I like the next figure actually better, but it's not labelled at all yet. HH}
\begin{figure}[htbp]
\begin{center}
...
\begin{figure}[htbp]
\centering
<<transform-illustration,echo=FALSE, fig.keep='none'>>=
x <- seq(-pi, pi, length=200)
y <- sin(x)
deriv <- cos(0)*x
df1 <- data.frame(x=x, y=y, deriv=deriv)
x <- seq(-pi/2, pi/2, length=15)
y <- sin(x)
ystart <- y-1
yend <- y+1
df2 <- data.frame(xstart=x, xend=x, ystart=ystart, yend=yend)

getSecantSegment <- function(x, l){
  secSlope <- -cos(x)
  temp <- seq(-pi, pi, .001) + x
  leftend <- temp[which.min(abs(sin(temp) - secSlope*(temp) - l/2))]
  rightend <- temp[which.min(abs(sin(temp) - secSlope*(temp) + l/2))]
  yintercept = sin(x) -secSlope*x 
  data.frame(xstart=leftend+x, xend=rightend+x, ystart=(yintercept + secSlope*(leftend+x)), yend = (yintercept + secSlope*(rightend+x)))
}


qplot(x=x, y=y, geom="line", data=df1, colour=I("grey50")) + theme_bw() + 
  geom_line(aes(y=y+1), colour="grey50", linetype=4) + geom_line(aes(y=y-1), colour="grey50", linetype=4) +
  geom_segment(data=df2[c(1, 3, 6, 8, 10, 13, 15),], aes(x=xstart, xend=xstart, y=ystart, yend=yend)) + 
#  geom_line(data=df1, aes(x=x, y=deriv), linetype=2) + 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), colour="blue", data=getSecantSegment(0, 2)) + 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), colour="blue", data=getSecantSegment(pi/8, 2)) + 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), colour="blue", data=getSecantSegment(pi/4, 2)) + 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), colour="blue", data=getSecantSegment(5*pi/8, 2)) + 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), colour="blue", data=getSecantSegment(3*pi/4, 2)) + 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), colour="blue", data=getSecantSegment(7*pi/8, 2)) + 
  geom_segment(aes(x=xstart, xend=xend, y=ystart, yend=yend), colour="blue", data=getSecantSegment(pi, 2)) + 
  geom_text(aes(x=xend-.1, y=yend+.1, label="Perceived\nwidth at\nx=0"), colour="blue", hjust=1, vjust=0, data=getSecantSegment(0, 2), inherit.aes=FALSE) + 
  geom_text( x=-.15, y=.35, label="theta", parse=TRUE, size=4, hjust=.5, vjust=.5) + 
  geom_line(aes(x=.6*cos(seq(3*pi/4, pi/2, length=20)), y=.6*sin(seq(3*pi/4, pi/2, length=20)))) + 
  coord_equal(ratio=1) + xlim(c(-pi, pi)) + ylim(c(-2.25, 2.25))

lambda <- seq(-2,2, by=0.01)
a <- 1
dframe1 <- data.frame(x=f(a)+lambda*fprime(a), y = f(a)-lambda)
a <- 0.5
dframe2 <- data.frame(x=f(a)+lambda*fprime(a), y = f(a)-lambda)
a <- 0
dframe0 <- data.frame(x=f(a)+lambda*fprime(a), y = f(a)-lambda)
  
qplot(x=x, y=y, geom="line", data=df1, colour=I("grey50")) + theme_bw() + 
  geom_line(aes(y=y+1), colour="grey50", linetype=4) + geom_line(aes(y=y-1), colour="grey50", linetype=4) +
  geom_segment(data=df2[c(1, 3, 6, 8, 10, 13, 15),], aes(x=xstart, xend=xstart, y=ystart, yend=yend)) + 
  geom_point(aes(x=x, y=y), size=1, colour="blue", data=dframe1) + 
  geom_point(aes(x=x, y=y), size=1, colour="blue", data=dframe2) + 
  geom_point(aes(x=x, y=y), size=1, colour="blue", data=dframe0) + 
  coord_equal(ratio=1)
@
\subfloat[1]{\includegraphics[width=.45\linewidth, keepaspectratio=TRUE]{figure/fig-transform-illustration1}}
\subfloat[2]{\includegraphics[width=.45\linewidth, keepaspectratio=TRUE]{figure/fig-transform-illustration2}}
\caption{The perceived width orthogonal to the slope differs from the length of the lines. \todo[inline]{include blue orthogonal lines for the whole function. That way we can also see the problem at the changing curvature better -- you'll need to first make the getSecant function more general. }
}
\end{figure}
...
\todo[inline]{for figure \ref{ytransform} I'm pretty sure that the fig.height=c(2,4,6) specification in knitr doesn't work.}
...
\caption{\label{ytransform} Changing aspect ratios changes the strength of the illusion and the effectiveness of the transformation}
\end{figure}

\begin{figure}[hbtp]
<<fixingit, dependson='data', echo=FALSE, results='asis',out.width='.3\\textwidth', fig.width=5, fig.height=3>>=
...
@
\caption{\label{firstfix} First fix of the sine illusion by using the factor of $1/\cos\left(\atan{|f^\prime(x)|}\right)$ (middle) and $\sqrt{1 + f^\prime(x)^2}$ (right) for adjusting the vertical extent of the lines.  }
\end{figure}

\begin{figure}
\centering
<<secondfix, dependson='data', echo=FALSE, out.width='0.49\\textwidth',  fig.width=5, fig.height=5>>=
dframe <- createSine(40, 0.5, f, fprime)
ar <- 2
...
\begin{figure}
\centering
<<thirdfix, dependson='data', echo=FALSE, out.width='0.49 \\textwidth'>>=
...
<<CtsMethodSetup, echo=FALSE, fig.keep='all'>>=
getSecantSegment <- function(x1, df, f, fprime){
  secSlope   <- -1/fprime(x1)
  ell.x1     <- df$ell[which.min(abs(df$x-x1))]/2
  ...
  df2 <- data.frame(x=x1, y=f(x1), deriv=fprime(x1),
                    sec.xstart=leftend, sec.xend=rightend, 
                    sec.ystart=(yintercept + secSlope*(leftend)),
<<<<<<< HEAD
                    sec.yend = (yintercept + secSlope*(rightend)),
                    ell.orig = 2*ell.x1)
=======
                    sec.yend = (yintercept + secSlope*(rightend)))
>>>>>>> 9e27162a738867a88719784e867eaf6a552ace15
...
getAdjLength<- function(df, center=TRUE){
  theta <- atan(df$deriv)  
  if(!center){
<<<<<<< HEAD
  ...
  df$seg.ystart <-  with(df, y+ell.orig/2*(1+cos(theta))-(sec.ell1+sec.ell2)/2*cos(theta))
  df$seg.yend   <-  with(df, y-ell.orig/2*(1+cos(theta))+(sec.ell1+sec.ell2)/2*cos(theta))
=======
  df$seg.ystart <-  with(df, y+ell.x1*(1+cos(theta))-sec.ell1*cos(theta))
  df$seg.yend   <-  with(df, y-ell.x1*(1+cos(theta))+sec.ell2*cos(theta))
  return(df)
  }  
  df$seg.ystart <-  with(df, y+sec.ell1/(sec.ell1+sec.ell2)*2*ell.x1)
  df$seg.yend   <-  with(df, y-sec.ell2/(sec.ell1+sec.ell2)*2*ell.x1)
>>>>>>> 9e27162a738867a88719784e867eaf6a552ace15
  df
}
...
library(multicore)
secantlines <- do.call("rbind", mclapply(seq(0, 2*pi, length=40), function(i) getSecantSegment(i, dframe, sin, cos)))
secantlines2 <- getAdjLength(secantlines, center=TRUE)
...
=======
names(dframe2) <- c("x", "y", "xstart", "xend", "ystart", "yend", "ell1", "ell2", "new.ystart", "new.yend", "type")
>>>>>>> 9e27162a738867a88719784e867eaf6a552ace15
...
qplot(x=xstart, xend=xend, y = ystart, yend=yend, geom="segment", data=dframe2, colour=type) +
  ...
  geom_segment(aes(x=sec.xstart, xend=sec.xend, y=sec.ystart, yend=sec.yend), data=secantlines, colour="blue") + 
  ggtitle("Untransformed Data") + theme(legend.position="bottom") 
...
qplot(x=x, xend=x, y=seg.ystart, yend=seg.yend, geom="segment", data=secantlines2, linetype=I(1)) + ggtitle("Adjusted Data") + coord_fixed(ratio=1) #+ geom_line(aes(x=x, y=sin(x) + 1/2), size=1) + geom_line(aes(x=x, y=sin(x) - 1/2), size=1)
=======
qplot(x=x, xend=x, y=0, yend=sec.ell1, geom="segment", data=secantlines, linetype=I(1)) + 
  geom_segment(aes(yend=-sec.ell2), linetype=4) + ggtitle("Extant Segment Length (Centered)") + coord_fixed(ratio=1) + ylim(c(-1,1))

qplot(x=x, xend=x, y=0, yend=sec.ell1+sec.ell2, geom="segment", data=secantlines, linetype=I(1)) + ggtitle("Extant Segment Length") + coord_fixed(ratio=1)

qplot(x=x, xend=x, y=seg.ystart, yend=seg.yend, geom="segment", data=secantlines, linetype=I(1)) + ggtitle("Adjusted Segment Length") + coord_fixed(ratio=1)

qplot(x=x, xend=x, y=0, yend=ell1, geom="segment", data=secantlines, linetype=I(1)) + 
  geom_segment(aes(yend=-ell2), linetype=4) + ggtitle("Extant Segment Length (Centered)") + coord_fixed(ratio=1) + ylim(c(-1,1))

qplot(x=x, xend=x, y=seg.ystart+y, yend=seg.yend+y, geom="segment", data=secantlines, linetype=I(1)) + ggtitle("Adjusted Data") + coord_fixed(ratio=1) 
>>>>>>> 9e27162a738867a88719784e867eaf6a552ace15
@
\end{subsection}
...
<<xtransform, echo=FALSE, results='asis', out.width=".31\\textwidth">>=
dframe <- createSine(80 , len=1, f=f, fprime=fprime)
dframe$a <- c(0, diff(dframe$ystart))
