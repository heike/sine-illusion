...
getSecantSegment <- function(x1, df, f, fprime, f2prime){
...
#   rightend   <- temp[which.min(abs(f(temp) - ell.x1 - secSlope*(temp-x1)))]
  ...
  #--- Approximation
  leftend <- (-(1+fprime(x1)^2) + sqrt((1+fprime(x1)^2)^2 - 2*f2prime(x1)*fprime(x1)*(fprime(x1)*f(x1)+ell.x1)))/(f2prime(x1)*fprime(x1))+x1 
  rightend <- (-(1+fprime(x1)^2) + sqrt((1+fprime(x1)^2)^2 - 2*f2prime(x1)*fprime(x1)*(fprime(x1)*f(x1)-ell.x1)))/(f2prime(x1)*fprime(x1))+x1
  #---
...
library(multicore)
secantlines <- do.call("rbind", mclapply(seq(0, 2*pi, length=40), function(i) getSecantSegment(i, dframe, sin, cos, function(x) -sin(x))))
...
qplot(x=xstart, xend=xend, y = ystart, yend=yend, geom="segment", data=dframe2, colour=type) +
  ...
  ggtitle("Untransformed Data") + theme(legend.position="bottom")  + xlab("") + ylab("")
@
\caption{The perceived length of the lines is a function of the extant width, that is, the width of the secant line at a given point. The secant lines are varying lengths (0.8 to 1.2), suggesting that to counter the illusion, the line lengths must be altered such that the extant widths are perceived to be equal.}
\end{figure}
\begin{figure}[htbp]\centering
...
qplot(x=x, y=sec.ell1, geom="line", data=secantlines) + 
  ...
  theme_bw()
@
\caption{The secant  line segments are not symmetrical around the center line, so symmetric correction as employed in Figure \ref{thirdfix} leads to simultaneous over- and under-correcting which causes the distortions seen above.\todo[inline]{Not sure about this claim, but seems likely... we'll see...}}
\end{figure}
\begin{figure}[htbp]\centering
