...
qplot(x=x, y=y, geom="line", data=dframe, colour=I("grey50")) + theme_bw() + 
  ...
  geom_text(aes(label=paste("theta", "%~~%", round(vangle/pi*180), "^o", sep=""),
                x=xstart/2+x/2-sign(vangle)*.1, y=y-sign(vangle)*.64+.04), data=dframe.1, parse=TRUE, hjust=.75, vjust=.5) + 
  xlim(c(-pi/8, 17*pi/8)) + ggtitle("Changes in Perceived Orthogonal Width due to Change in Secant Angle")
@
